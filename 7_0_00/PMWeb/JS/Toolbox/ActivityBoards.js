    ...
    $('.DivNewColText input').keydown(function (e) {
        if (e.key === 'Enter') {
            ...
            $.ajax({
                ...
                data: "{'ColumnName':'" + this.value.replace("\\", "\\\\").replace("'", "\\'") + "'}",
                dataType: "json",
                async: true,
...
function EditColKeyDown(sender,e) {
    if (e.key === 'Enter') {
        ...
        $.ajax({
            ...
            url: "AjaxService.aspx/EditActivityBoardColumnname",
            contentType: "application/json; charset=utf-8",
            data: "{'ColId':" + $(sender).parents('.ABCol')[0].id.replace('ABCol_', '') + ",'ColumnName':'" + Colvalue + "'}",
            dataType: "json",
            async: true,
            success: function (data) {
...
function ItemDropped(event, ui) {
    ...
    $.ajax({
        ...
        contentType: "application/json; charset=utf-8",
        data: "{'TaskId':" + ui.item[0].id.replace('ABRow_', '') + ",'FromColumnId':" + ui.item.attr('colid') + ",'ToColumnId':" + ToColumnId + ",'Position': " + Position + "}",
        dataType: "json",
        async: true,
        success: function (data) {
            if (data.d == 'F') alert('The action could not be taken, please refresh the page and try again.');
...
function itemSorted(event, ui) {
    ...
    $.ajax({
        ...
        contentType: "application/json; charset=utf-8",
        data: "{'TaskId':" + ui.item[0].id.replace('ABRow_', '') + ",ToPosition:" + ToPosition + "}",
        dataType: "json",
        async: true,
        success: function (data) {
            if (data.d == 'F') alert('The action could not be taken, please refresh the page and try again.');
...
function ColSorted(event, ui) {
    ...
    $.ajax({
        ...
        contentType: "application/json; charset=utf-8",
        data: "{'ColId':" + ui.item[0].id.replace('ABCol_', '') + ",ToPosition:" + ToPosition + "}",
        dataType: "json",
        async: true,
        success: function (data) {
            if (data.d == 'F') alert('The action could not be taken, please refresh the page and try again.');
    ...
    divRow.find('.inputAddTask').focus().keydown(function (e) {
        if (e.key === 'Enter') {
            ...
            $.ajax({
                ...
                contentType: "application/json; charset=utf-8",
                data: "{'TaskName':'" + Currvalue.replace("\\","\\\\").replace("'", "\\'") + "','ColId':" + ColId + ",'Position':" + Position + "}",
                dataType: "json",
                async: true,
    ...
    if (!DateNotSelected) {
        if (currentTextBox != null) {
            ...
            $.ajax({
                ...
                contentType: "application/json; charset=utf-8",
                data: "{'TaskId':" + TaskId + ",'DueDate':'" + args.get_newDate().toMMDDYYYYString() + "'}",
                dataType: "json",
                async: false,
    ...
    switch (commandName) {
        case 'MarkDone':
            ...
            $.ajax({
                ...
                contentType: "application/json; charset=utf-8",
                data: "{'TaskId':" + TaskId.toString() + ",'IsDone':" + IsDone + "}",
                dataType: "json",
                async: true,
                success: function (data) {
                    if (data.d == 'F') alert('The action could not be taken, please refresh the page and try again.');
