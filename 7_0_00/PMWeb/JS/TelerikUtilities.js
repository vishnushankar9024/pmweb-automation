...
$(document).ready(function () {
    ...
    //    botmargin = $(".MobileFixedMenu")[0].clientHeight;
    if ($(".bottomtoolbar").length == 1 && $(".bottomtoolbar").css("display") != "none")
        botmargin = botmargin + $(".bottomtoolbar")[0].offsetHeight;
    $(".divContentHolder").css({
        "margin-bottom": (botmargin).toString() + "px"
    });
    $(".divContentHolderHome").css({ "margin-bottom": (botmargin).toString() + "px" });
});
...
function OpenRecentDocumentsPopup(objectType) {
    if (typeof HideFlyoutMenu == "function")
        HideFlyoutMenu();
    if (objectType == 'AllRecordTypes') {
        var btnRecentId = $("[id$=btnMenuRecent]")[0].id
        $('.nav-link').each(function () {
            if (this.id != btnRecentId)
                this.className = this.className.replace(' active', '');
        });
        if ($("[id$=btnMenuRecent]")[0].className.indexOf('active') < 0)
            $("[id$=btnMenuRecent]")[0].className = $("[id$=btnMenuRecent]")[0].className + ' active';
    }
    if (objectType != '' && wndRecentRecords == null) {
        wndRecentRecords = window.radopen('RecentDocuments.aspx?objecttype=' + objectType);
        wndRecentRecords.add_close(WindowRecentClosed);
    }
    if (wndRecentRecords == null && objectType == '') return;
    $(".rwBodyLeft").remove()
    $(".rwBodyRight").remove()
    $(".rwFooterRow").remove();
    $(".rwTitleRow").remove();
    $(".rwWindowContent").css("width", "100%");
    wndRecentRecords.GetTitlebar().parentElement.hidden = true
    wndRecentRecords.set_modal(false);
    var browserWidth = $telerik.$(window).width();
    var browserHeight = $telerik.$(window).height();
    var toolbarTop = $("[id$=toolbartop]")[0];
    var tbsHomeDocument = $('#ctl00_CPH1_tbsHomeDocument')[0];
    var HomeTabTabsMobile = $(".HomeTabTabsMobile")[0];
    var top = 0;
    var botmargin = 0;
    if ($(".bottomtoolbar").length == 1 && $(".bottomtoolbar").css("display") != "none")
        botmargin = $(".bottomtoolbar")[0].offsetHeight;
    else if ($(".MobileFixedMenu").length == 1 && $(".MobileFixedMenu").css("display") != "none") {
        botmargin = 36;
    }
    if (isMobileScreen() && $('.drawer').is(":visible") == false) {
        wndRecentRecords.setSize(browserWidth, browserHeight - 36);
        wndRecentRecords.moveTo(0, 0);
        $('#drawerBackdrop').removeClass("backdrop-activated");
        $('.drawer').removeClass("activated")
    }
    else {
        if (tbsHomeDocument != null) {
            top = tbsHomeDocument.offsetHeight;
            wndRecentRecords.setSize(HomeTabTabsMobile.offsetWidth - $('.drawer')[0].offsetWidth, browserHeight - botmargin - top);
            wndRecentRecords.moveTo($('.drawer')[0].offsetWidth, tbsHomeDocument.offsetHeight);
        }
        else {
            top = toolbarTop.offsetHeight;
            wndRecentRecords.setSize(toolbarTop.offsetWidth - toolbarTop.offsetLeft, browserHeight - botmargin - top);
            wndRecentRecords.moveTo(toolbarTop.offsetLeft, toolbarTop.offsetHeight);
        }
    }
    $(".RadWindow").css("position", "fixed");
    $(".RadWindow").css("top", top + "px");
    if ($('.drawer').is(":visible") == false) {
        $(".RadWindow").css("top", "0");
        $(".RadWindow").css("bottom", "36px");
    }
    $(".TelerikModalOverlay").remove();
    wndRecentRecords.set_behaviors(0);
    return false;
}
...
function TelerikWindowResizeEnd() {
    ...
    if (new Date() - RTime < RDelta) {
        ...
        if ($(".bottomtoolbar").length == 1 && $(".bottomtoolbar").css("display") != "none")
            botmargin = botmargin + $(".bottomtoolbar")[0].offsetHeight;
        $(".divContentHolder").css({ "margin-bottom": botmargin.toString() + "px" });
        $(".divContentHolderHome").css({ "margin-bottom": botmargin.toString() + "px" });
        OpenRecentDocumentsPopup('');
    }

}
//function MainToolbarCreated(sender,args){
//    var a=1
//    ResizemainToolbar()
//}
//function ResizemainToolbar() {
//    if ($(".ToolBar").length == 0) return;
//    MainToolbar = $(".ToolBar")[0];
//    var screenwidth = document.documentElement.clientWidth;
//    debugger;
//    if (MainToolbar.offsetLeft + MainToolbar.offsetWidth  > screenwidth) {
//        $(MainToolbar).find("td").each(function () {
//            var td = $(this)[0];
//            if (td.className.indexOf("HideOnMobileToolbar") > 0)
//                $(td).css({ "display": "none" });
//            if ($(td).find(".RadToolBar").length > 0)
//                $(td).find("li").each(function () {
//                    var li = $(this)[0];
//                    if (li.className.indexOf("HideOnMobileToolbar") > 0)
//                        $(li).css({ "display": "none" });
//                    if (li.className.indexOf("ToolbarMobileMenu") > 0) {
//                        $(li).attr( "style", "display:inline-block !important" );
//                    }
//                })
//        })
//    }
...
function SetGridHeight(ClientID, IsVirtual) {
    ...
    if (ClientID != 'ctl00_CPH1_ucRoles_rdgRoles') {
        var GridFixedElementsHeight = 2;// 2 for borders, 
        if ($(".bottomtoolbar").length == 1 && $(".bottomtoolbar").css("display") != "none")
        ...
        if ($(".toolbartop:visible").length > 0)
            FixedElementsHeight = FixedElementsHeight + $(".toolbartop:visible").height();
