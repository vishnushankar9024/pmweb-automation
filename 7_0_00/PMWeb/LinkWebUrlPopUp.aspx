    ...
    <form id="form1" runat="server">
        <script type="text/javascript">
            ...
            function maintoolBar_Clicked(sender, args) {
                //debugger;
                //var hfWebUrl = window.parent.document.querySelector("#ctl00_CPH1_DocumentAttachments_hfWebUrl");
                //hfWebUrl.value = document.querySelector('#txtDescription').value;
                var hfWebUrl= $(window.parent.document).find("input[id$='hfWebUrl']");
                //var DescriptionValue = document.querySelector('#txtDescription').value
                var DescriptionValue = $("[id$=txtDescription]").val();
                hfWebUrl.val(DescriptionValue);
                var urlParams = new URLSearchParams(window.location.search);
                if (urlParams.has('isSharePoint')) {
                    //window.parent.document.querySelector(".SaveSharePoint").click();
                    $(window.parent.document).find(".SaveSharePoint")[0].click();
                } else
                {
                    $(window.parent.document).find(".SaveUrl")[0].click();
                    //window.parent.document.querySelector(".SaveUrl").click();
                }
                var chk = $("[id$=chkNewTab]")[0];
                //var chk = document.querySelector("#chkNewTab");
