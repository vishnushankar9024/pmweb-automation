...
function LoadFrame(url, ocs) {
    ...
    h -= 50; // Adjust for modal header
    $(".modal-content", modal).css({ "height": h, "max-height": h, 'margin-top': '-10px', 'margin-left': '-10px', 'overflow': 'hidden' });
    h -= 60; // Adjust for footer
    h = h >= 500 ? h : h;
    $(".modal-body", modal).css({ "height": h, "max-height": h, 'overflow': 'auto', "flex": "1 1 auto"});
    modal.modal();
    $("#pmwebRecordFrame").remove();
	 // Set modal dialog to use flexbox for full-screen fit
    $(".modal-dialog", modal).css({
        "display": "flex",
        "flex-direction": "column",
        "height": "100vh",
        "width": "100vw",
        "max-width": "100%", // To handle different screen resolutions
        "margin": "0",
        "padding": "0"
    });
    ...
    _iframe = $('<iframe>', {
        ...
        // Remove the MenuContainer
        iframeContent.find('.MenuContainer').remove();

        // Adjust toolbar to sticky positioning
        var $toolbar = iframeContent.find('.toolbartop');
        $toolbar.css({
            'position': 'sticky', // Removed !important as it’s ineffective in jQuery .css()
        });

        // Calculate and set padding for ContentPane based on toolbar and RadTabStrip
        var contentPadding1 = $toolbar.outerHeight() || 50;
        console.log(contentPadding1);

        var $radTabStrip = iframeContent.find('.RadTabStrip_Default');
        var contentPadding = contentPadding1 + $radTabStrip.outerHeight();
        ...
        // Dynamically adjust the iframe height if needed
        setTimeout(function () {
...
function setFrameHeight(obj, url, ocs) {
    ...
    obj.css({
        ...
        height: modalBodyHeight - 5 // Adjust if needed
    });
