$(document).ready(function () {
    ...
    if (global.TypeId == 174) {
        $('#ctl00_CPH1_CustomFormDetails1_txtPHRecordNumber').hide();
        $('#ctl00_CPH1_CustomFormDetails1_txtPHRecordNumber').parent().prev('td').hide();
    }
    ...
    if (typeof ($("#btnRevision").val()) != 'undefined') {
        ...
        if ((global.CustomFormId == "" || typeof (global.CustomFormId) == 'undefined') && (customFormId == 'Select Custom Form ...' || customFormId == '' || typeof (customFormId) == 'undefined')) {
            ...
            $.ajax({
                url: 'PMWebhelper/HMC/GetStepNumber/?customformId=' + global.CustomFormId,
                dataType: "html",
                type: "GET",
...
function CreateBISessionSubmit() {
    ...
    //}
    //
    //var url = 'PMWebhelper/HMC/SetBISession/?customformId=' + global.CustomFormId + '&wfaction=submit';
    //$.ajax({
    //    url: url,
    //    dataType: "html",
    //    type: "GET",
...
function CreateBISessionWorkflowSave() {
    ...
    //    var url = 'PMWebhelper/HMC/SetBISession/?customformId=' + global.CustomFormId + '&wfaction=' + vl;
    //    $.ajax({
    //        url: url,
    //        dataType: "html",
    //        type: "GET",
...
function CreateBIReport() {
    ...
    //    }
    //}
    //var url = 'PMWebhelper/HMC/CustomformFinalApproval/?userId=' + global.User.Id;
    //$.ajax({
    //    url: url,
    //    dataType: "html",
    //    type: "GET",
...
function AttachBiForAll() {
    var url = 'pmwebhelper/hmc/AttachAllBi';
    $.ajax({
        url: url,
        dataType: "html",
        type: "GET",
...
function CreateBIWorkflowSave() {
    ...
    //    }
    //    var vl = $('input[type="radio"][name="ctl00$CPH1$WorkflowDocument1$Approve"]:checked').val();
    //    var url = 'PMWebhelper/HMC/CustomformFinalApproval/?customformId=' + global.CustomFormId + '&wfaction=' + vl + '&userId=' + global.User.Id;
    //    $.ajax({
    //        url: url,
    //        dataType: "html",
    //        type: "GET",
...
function ClickRevisionHandler() {
    ...
    $.ajax({
        url: 'PMWebhelper/HMC/CustomformReVersion/?customformId=' + global.CustomFormId + '&rev=' + rev,
        dataType: "html",
        type: "GET",
        contentType: 'application/json;charset=utf-8',
        async: true,
...
var Risk = {
    ...
    RiskClose: function (Id) {
        $.ajax({
            url: 'PMWebhelper/HMC/RiskClose/?Id=' + Id,
            dataType: "html",
            type: "GET",
            contentType: 'application/json;charset=utf-8',
            async: true,
    ...
    Init: function () {
        ...
        if (customFormId != 'Select Custom Form ...' && customFormId != '' && typeof (customFormId) != 'undefined') {
            $.ajax({
                ...
                url: 'PMWebhelper/HMC/GetCorrespondenceButtons/?customformId=' + global.CustomFormId,
                dataType: "html",
                type: "GET",
                contentType: 'application/json;charset=utf-8',
                async: true,
                ...
                $('<button />', {}).text('Yes')
                    .click(function () {
                        ...
                        $.ajax({
                            ...
                            url: 'PMWebhelper/HMC/CorrespondenceButtonAction/?customformId=' + global.CustomFormId + '&func=' + button.Func + '&userid=' + global.User.Id,
                            dataType: "html",
                            type: "GET",
                            contentType: 'application/json;charset=utf-8',
                            async: true,
    ...
    Init: function () {
        ...
        if (customFormId != 'Select Custom Form ...' && customFormId != '' && typeof (customFormId) != 'undefined') {
            $.ajax({
                ...
                url: 'PMWebhelper/HMC/GetPaymentApplicationButtons/?customformId=' + global.CustomFormId,
                dataType: "html",
                type: "GET",
                contentType: 'application/json;charset=utf-8',
                async: true,
                ...
                $('<button />', {}).text('Yes')
                    .click(function () {
                        ...
                        $.ajax({
                            ...
                            url: 'PMWebhelper/HMC/PaymentApplicationButtonAction/?customformId=' + global.CustomFormId,
                            dataType: "html",
                            type: "GET",
                            contentType: 'application/json;charset=utf-8',
                            async: true,
...
$(function () {
    ...
    if ($('#ctl00_CPH1_WorkflowDocument1_pnlWorklfowActions').length > 0 || $('#pnlStepActions').length > 0) {
        GenerateDynamicButtons.Init();
    }
...
function GetUser() {
    $.ajax({
        url: 'cmcs/pmweb/custom/pmwebhelper.aspx/GetUser',
        dataType: "json",
        type: "GET",
        contentType: 'application/json;charset=utf-8',
        async: false,
...
function ShowWorkflowNestedRows() {
    ...
    setTimeout(function () {
        var pendingRowIdx = 0;
        var pendingColumnIdx = 0;
        if ($('#' + WorkflowDocumentGrid).length > 0) {
            var tdIdx = 0;
            var tdPrevIdx = 0;
            var _grid = $('#' + WorkflowDocumentGrid);
            $('tbody tr:has(input.rtlExpand)', _grid).each(function () {
                var tr = $(this);
                $('.rtlExpand', tr).click();
            });
        }
        $('tbody tr', $('#' + WorkflowDocumentGrid)).each(function () {
            if ($(this).css("background-color") == "rgb(255, 172, 132)" && pendingRowIdx == 0) {
                pendingRowIdx = $("#" + WorkflowDocumentGrid + " tbody tr").index(this);
            }
        });
        $('tbody tr:has(input.rtlCollapse)', $('#' + WorkflowDocumentGrid)).each(function () {
            $('.rtlCollapse', $(this)).click();
        });


        var pendingRow = $('tbody tr:eq(' + pendingRowIdx + ')', $('#' + WorkflowDocumentGrid));
        var tr = pendingRow.prevAll('tr:has(input.rtlExpand)')[0];
        tr = $(tr);
        var button = $('.rtlExpand', tr);
        var td = button.parent();
        pendingColumnIdx = td.index() + 1;

        var ct = 0;
        while (pendingColumnIdx > 0 && ct <= 10) {
            ct++;
            tr = pendingRow.prevAll('tr:has(input.rtlExpand)')[0];
            tr = $(tr);
            button = $('.rtlExpand', tr);
            td = button.parent();
            if (td.index() == pendingColumnIdx - 1) {
                pendingColumnIdx--;
                button.click();
            }
            else if (ct > 1) {
                var rowsBetween = tr.nextUntil(pendingRow);
                console.log(rowsBetween);
                //pendingRow.remove();
            }
            pendingRow = tr;
        }
...
var trCt = 0;
function CheckRecursiveWorkflowSteps(objTr) {
    var prevTr = $(objTr, $('#' + WorkflowDocumentGrid)).prev();
    trCt++;
    if (prevTr && trCt <= 15) {
        $('td:has(.rtlL2)', prevTr).each(function () {
            $('button', $(this)).click();
        });
        CheckRecursiveWorkflowSteps(prevTr);
    }
}
...
function HideWorkflowLog() {
    if (global.User.Email != "" && $('#' + WorkflowDocumentGrid).length > 0 && (global.User.Email.toLowerCase().indexOf("@hamad.qa") < 0 && global.User.Email.toLowerCase().indexOf("@hmc.org.qa") < 0)) {
        ...
        var stepColumnLastIndex = 0;
        var ramColumnIndex = 0;
        var ramColumnLastIndex = 0;
        ...
        ramColumnLastIndex = $("#" + WorkflowDocumentGrid + " th").length - ramColumnIndex;
        stepColumnLastIndex = $("#" + WorkflowDocumentGrid + " th").length - stepColumnIndex;

        $("#" + WorkflowDocumentGrid + " tr").each(function () {
            var ramColumnIndexTmp = $('td', $(this)).length - ramColumnLastIndex;
            var stepColumnIndexTmp = $('td', $(this)).length - stepColumnLastIndex;
...
function BindProjectContactDropDown(objDropdown) {
    ...
    if (select) {
        MultiAjaxAutoComplete("#customizedContact", 'pmwebhelper/hmc/GetHMCContacts');
    }
}
...
function MultiAjaxAutoComplete(element, url) {
    ...
    if (txtCustomizedCC.val() != "") {
        $.ajax({
            url: 'pmwebhelper/hmc/GetHMCContactsByEmail?emails=' + txtCustomizedCC.val(),
            dataType: "json",
            type: "GET",
            contentType: 'application/json;charset=utf-8',
            async: true,
            ...
            success: function (data) {
                $.each(data, function (i) {
                    ...
                    val = data[i].id;
                    txt = data[i].text;
                    var $option = $('<option selected>' + data[i].text + '</option>').val(data[i].id).attr('title', data[i].title);
...
function formatRepoSelection(repo) {
    console.log(repo);
    return repo.text;
    //return '<span class="select2-selection__choice__remove" role="presentation">×</span>' + repo.text;
}
...
//{
//    $.ajax({
//        url: 'pmwebhelper/hmc/GetProjectContacts?projectId=' + projectId,
...
var GenerateDynamicButtons = {
    Init: function () {
        var customFormId = $('#ctl00_CPH1_ddlCustomForms_Input').val();
        GenerateDynamicButtons.Render();
        if (customFormId != 'Select Custom Form ...' && customFormId != '' && typeof (customFormId) != 'undefined') {
            $.ajax({
                //url: 'http://localhost:58318/PMWebWorkflowButtons/GetGenerateButtons/?customformId=' + global.CustomFormId,
                url: 'PMWebhelper/PMWebWorkflowButtons/GetGenerateButtons/?customformId=' + global.CustomFormId + '&UserId=' + global.User.Id,
                dataType: "html",
                type: "GET",
                contentType: 'application/json;charset=utf-8',
                async: true,
                ...
                success: function (data) {
                    if (data != -1) {
                        GenerateDynamicButtons.GenerateButtons(JSON.parse(data));
                    }
                    else {
    ...
    GenerateButtons: function (buttons) {
        if (buttons.length > 0) {
            ...
            //var table = jQuery('tr:eq(0) td:eq(0)', $('#WorkflowDocument'));

            var table = jQuery('#ctl00_CPH1_WorkflowDocument1_pnlWorklfowActions').length > 0 ? jQuery('table table tr:eq(0)', '#ctl00_CPH1_WorkflowDocument1_pnlWorklfowActions') : jQuery('tr.ToolBar', '#pnlStepActions');
            var tdNew = jQuery('<td/>', {
                id: "tdDynamicButtons",
                css: {
                    width: '188px'
            ...
            tdNew.attr('valign', 'top');
            tdNew.attr('align', 'left');

            if (jQuery('#ctl00_CPH1_WorkflowDocument1_pnlWorklfowActions').length <= 0) {
                tdNew.attr('rowspan', '2');
            }
            //tdNew.appendTo(table);
            tdNew.appendTo(table);
        ...
        $.each(buttons, function (i) {
            var button = buttons[i];
            $('<br /><input type="button" value="' + button.Text + '" style="width:250px;" /><br />').click(function () {
                if (button.Text == "Request for Information Log") {
                    window.open('NIPortal/pmweb/RequestforInformation/' + global.CustomFormId, 'blank', 'scrollbars=yes,toolbar=no,width=990,height=500');
                    return;
                }
                var winW = window.innerWidth;
                var winH = window.innerHeight;
                ...
                var linkedRecord = "";
                if (button.LinkedRecord && button.LinkedRecord.length > 0) {
                    linkedRecord = JSON.parse(button.LinkedRecord);
                    if (linkedRecord.length > 0) {
                        var tr;
                        var table = $('<table />');
                        for (var i = 0; i < linkedRecord.length; i++) {
                            tr = $('<tr/>');
                            tr.append("<td><a href='" + linkedRecord[i].DocumentLink + "' target='_top' style='color:#FFF'>" + linkedRecord[i].Description + "</a></td>");
                            table.append(tr);
                        }
                        $("#dialogboxbodyDynamicButtons").append(table);
                    }
                }
                $('#dialogboxfootDynamicButtons').html('');
                var canCreateRecord = true;
                if (button.LinkedRecord && button.LinkedRecord.length > 0 && linkedRecord.length > 0) {
                    canCreateRecord = button.CanCreateMultipleReply == 'yes';
                }
                if (canCreateRecord) {
                    $('<button />', {}).text('Yes')
                        .click(function () {
                            GenerateDynamicButtons.Close();
                            $.ajax({
                                //url: 'http://localhost:58318/PMWebWorkflowButtons/DoButtonAction/?CustomFormId=' + global.CustomFormId + '&MapFormId=' + button.Func + '&userid=' + global.User.Id,
                                url: 'PMWebhelper/PMWebWorkflowButtons/DoButtonAction/?CustomFormId=' + global.CustomFormId + '&MapFormId=' + button.Func + '&userid=' + global.User.Id,
                                dataType: "html",
                                type: "GET",
                                contentType: 'application/json;charset=utf-8',
                                async: true,
    ...
    Close: function () {
        document.getElementById('dialogboxDynamicButtons').style.display = "none";
        document.getElementById('dialogoverlayDynamicButtons').style.display = "none";
    }
}
/****************************************** End of Generate Dynamic Buttons ***************************************/