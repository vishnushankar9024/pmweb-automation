...
function droppedOnGroup(sender, args) {
    ...
    if (dest) {
        var destNodeValue = dest.get_value();
        if (destNodeValue.indexOf("_G") < 0) {
            args.set_cancel(true);
            return;
        }
        for (var i = 0; i < nodes.length; i++) {
            if (nodes[i].get_value() == destNodeValue) {
                args.set_cancel(true);
                return;
            }
        }
...
function onClientDoubleClick(sender, args) {
    ...
    if (nodeValue.indexOf("_I") > 0) {
        window.location = "Items.aspx?Id=" + nodeValue.replace(/\s/g, "").replace("_I", "");
    }
...
function AddFirstItemNode(sender, args) {
    switch (args.get_item().get_value()) {
        case "AddRootNode":
            var tree = $find($("[id$=treeGroupsAndItems]")[0].id);
            tree.trackChanges();
...
function onClientContextMenuItemClicking(sender, args) {
    ...
    switch (menuItem.get_value()) {
        ...
        case "NewItem":
            var nodeValue = treeNode.get_value();
            window.location = "Items.aspx?IsNew=true&GroupId=" + nodeValue.replace(/\s/g, "").replace("_G", "");
            args.set_cancel(true);
            break;
        case "Delete":
            if (nodes.length > 0) {
            ...
            //You are about to delete the selected groups, their sub-groups and all of the items within those groups, and the selected items. This process cannot be undone.\nAre you sure you wish to continue?
            ...
            result = confirm(Msg_DeleteGroup_SubGroup);
               ...
               // "You are about to delete the selected items. This process cannot be undone.\nAre you sure you wish to continue?"
...
function setMenuItemsState(menuItems, treeNode) {
    ...
    if (nodes.length > 1) {
        ...
        for (var i = 0; i < nodes.length; i++) {
            ...
            else if (nodes[i].get_value().indexOf("_G") > 0) {
                containsGroup = true;
            }
            else if (containsGroup && containsItem) break;
        }
        for (var i = 0; i < menuItems.get_count(); i++) {
            ...
            switch (menuItem.get_value()) {
                case "Rename":
                    ...
                    break;
                case "OpenItem":
                    menuItem.set_visible(containsItem && !containsGroup);
                    if (!(containsItem && !containsGroup))
                    {
...
function comboLoad(sender, eventArgs) {
    var tree = sender.get_items().getItem(0).findControl("treeItemGroups");
    var text = "";
...
function nodeClicking(sender, args) {
    var comboBox = $find($("[id$='ddlItemGroup']")[0].id);
...
function ClientNodeEdited(sender, args) {
    var tree = $find($("[id$=treeGroupsAndItems]")[0].id);
    var node = args.get_node();
    var Val = node.get_attributes(0).getAttribute("IsNew");
    if ((Val == "True" || Val == "NEWLIST") && node.get_text() == "") {
        tree.trackChanges();
        var parent = node.get_parent();
        alert(Msg_UnamedFolder); 
