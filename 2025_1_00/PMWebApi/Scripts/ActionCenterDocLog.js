...
function RedirectToPMweb() {
    if (isProduction == true) {
        window.top.location.href = "https://pmweb.hamad.qa/pmweb/";
    }
}
...
function ShowTeamInputDetails(docId, userId) {
    CheckSession(function () {
        $.ajax({
            ...
            type: "GET",
            contentType: 'application/json;charset=utf-8',
            data: { documentId: docId, userId: userId },
            beforeSend: function () {
                $('#pleaseWaitDialog').modal();
            },
            complete: function () {
            ...
            success: function (data) {
                ...
                $('.modal-body', modal).animate({ scrollTop: $('.modal-body', modal).offset().top });
                modal.modal();
...
function GetAttachmentsByRecordId(recordId, objectTypeId, documentId) {
    ...
    CheckSession(function () {
        $.ajax({
            ...
            type: "GET",
            contentType: 'application/json;charset=utf-8',
            data: { recordId: recordId, objectTypeId: objectTypeId, documentId: documentId },
            beforeSend: function () {
                $('#pleaseWaitDialog').modal('show');
            },
            complete: function () {
...
function GetAttachmentsActionCenter(projectId, recordId, DocId, Obj) {
    CheckSession(function () {
        $.ajax({
            ...
            type: "GET",
            contentType: 'application/json;charset=utf-8',
            data: { projectId: projectId, recordId: recordId, DocId: DocId },
            beforeSend: function () {
                $('#pleaseWaitDialog').modal('show');
            },
            complete: function () {
...
function GetRecord(url, ocs, checkPMWebSession) {
    CheckSession(function () {
        if (ocs) {
            $.ajax({
                url: 'https://pmweb.hamad.qa/OCS/CeateSession.aspx?username=' + global.pmwebUsername + '&userid=' + global.pmwebUserId,
                dataType: "html",
                type: "GET",
                contentType: 'application/json;charset=utf-8',
                beforeSend: function () {
...
function setFrameHeight(obj, url, ocs) {
    ...
    obj.css({
        ...
        height: $(".modal-body", modal).height()
    });

    frameContentHeight = obj.get(0).contentDocument.body.scrollHeight + 100;
    frameContentWidth = obj.get(0).contentDocument.body.scrollWidth + 100;
    if (url.toLowerCase().indexOf('pmwebhelper') < 0) {
        frameContentHeight = frameContentHeight - 50;
        frameContentHeight = frameContentHeight >= 720 ? frameContentHeight : 720;
    }
    obj.css({ "height": frameContentHeight + 'px' });
    obj.attr("height", frameContentHeight);
    obj.css({ "width": frameContentWidth + 'px' });
    ...
    if (!ocs) {
        ...
        onElementHeightChange(obj.get(0).contentDocument.body, function () {
            frameContentHeight = obj.get(0).contentDocument.body.scrollHeight;
...
function GetActionedDocuments(dataId, dtStart, dtEnd) {
    CheckSession(function () {
        $.ajax({
            ...
            type: "GET",
            contentType: 'application/json;charset=utf-8',
            data: { dataId: dataId, dtStart: dtStart, dtEnd: dtEnd },
            beforeSend: function () {
                $('#pleaseWaitDialog').modal('show');
            },
            complete: function () {
...
function InitDateRangePicker(dataId, dtStart, dtEnd, datePicker) {
    var active = '';
    if (dataId == 2) {
        active = "active";
    }
...
function setHeight(obj, isActionCenter) {
    $(obj).css({ "height": calcDataTableHeight(isActionCenter) + "px" });
}
...
function AddActionCenterActionedBadge(data) {
    //global.getAjax(global.url.GetActionedDocumentsAjax, { '_': Math.random(), 'dtStart': startDate, 'dtEnd': endDate }, function (_data) {
...
function AddActionCenterBadge(data) {
    global.getAjax(global.url.GetPendingDocumentsAjax, { '_': Math.random() }, AddActionCenterBadgeCallback);
...
function CloseSearch() {
    $("#panelsearch").slideUp();
    setTimeout(function () { $("#panelsearch").hide(); }, 200);
}
...
function LoadDocumentLog(e) {
    ...
    var _isProjectPhaseEnabled = Boolean($("#chkProjectPackage").is(':checked'));
    ...
    documentLogRequest = $.ajax({
        ...
        type: "POST",
        data: {
            fromDate: _fromDate, toDate: _toDate, projects: _projects, forms: _forms, discipline: _discipline, workflowstatus: _workflowstatus,
            subject: _subject, originReg: _originReg, subjectmatter: _subjectmatter, category: _category, from: _from, to: _to, docType: _docType,
            fileName: _fileName, createdBy: _createdBy, Gateway: _gateway, pendingWith: _pendingWith, listboxdepartments: _listboxdepartments,
            outref: _outref, recno: _recno, isProjectPhaseEnabled: _isProjectPhaseEnabled
        },
        cache: false,
        async: true,
        beforeSend: function () {
...
function selectAllForm() {
    ...
    var all = $.map(multiselect.dataSource.data(), function (dataItem) {
        if (dataItem.Name != "Memo" && dataItem.StrId != "10124" && dataItem.StrId != "10097" && dataItem.StrId != "10137" && dataItem.StrId != "10099") {
            return dataItem.StrId;
        }
    });
    multiselect.value(all);
    multiselect.trigger("change");
...
function SetCustomApplicateViewer(userId, applicationName) {
    $.ajax({
        ...
        type: "GET",
        contentType: 'application/json;charset=utf-8',
        data: { userId: userId, applicationName: applicationName, "_": Math.random() },
        async: true,
        cache: false,
        success: function () {
...
function OpenCloseWindow(url) {
    var rptWnd = window.open(url, 'blank', 'scrollbars=yes,toolbar=no,width=990,height=500');
    if (rptWnd) {
        rptWnd.onload = function () {
            rptWnd.close();
        };
    }
}
...
var filesSelected = [];
var docId = 0;
function OpenMoveCopyWindow() {
    var wnd = $('#copyMoveWindow').data('kendoWindow');
    wnd.refresh({
        url: moveUrl + '?DocId=' + docId
    });
    kendo.ui.progress(wnd.element, true);
    wnd.center().open();
}

function GetKendoListBoxSelectedItem(listboxId) {
    var listbox = $('#' + listboxId).data('kendoListBox');
    var items = listbox.select();
    var dataItems = [];
    for (var i = 0; i < items.length; i++) {
        dataItems.push(listbox.dataItem(items[i]));
        listbox.remove(items[i]);
    }
    return dataItems;
}

function GetKendoTreeviewSelectedNode(treeviewId) {
...
function MoveListBoxToTreeview(listboxId, treeviewId) {
    var treeview = $('#' + treeviewId).data('kendoTreeView');

    var tvNode = GetKendoTreeviewSelectedNode(treeviewId);
    var lbItems = GetKendoListBoxSelectedItem(listboxId);

    if (tvNode.item && tvNode.item.UploadFiles === true) {
        treeview.expand(tvNode);
        console.log(tvNode);
        if (tvNode.item.hasChildren) {
            var child = tvNode.item.children.data();
            console.log(child);
        }
        filesSelected[tvNode.item.id] = lbItems;

        for (var i = 0; i < lbItems.length; i++) {
            treeview.append({ id: lbItems[i].Id, Name: lbItems[i].Name, IsFile: true }, tvNode.selected);
        }

        $(tvNode.selected).find('.folder').removeClass('fa-folder');
        $(tvNode.selected).find('.folder').addClass('fa-folder-open');
    }
}

function tvFolderOnSelect(e) {
    var item = e.sender.dataItem(e.node);
    console.log($(e.node));
    if (item.UploadFiles === false) {
        //$(e.node).tooltip({
        //    content: "You are not allowed to upload files in the selected folder...",
        //    show: true//{ effect: "blind", duration: 800 }
        //});
        ...
        $('#btnMove').removeProp('disabled');
    }
}

function tvFolderOnExpand(e) {
    var fIcon = $(e.node).find('.folder');
    fIcon.removeClass('fa-folder');
    fIcon.addClass('fa-folder-open');
}
function tvFolderOnCollapse(e) {
    var fIcon = $(e.node).find('.folder');
    fIcon.removeClass('fa-folder-open');
    fIcon.addClass('fa-folder');
}
//function GetSelected
function DownloadDocument() {
    var documentDownloads = [];
    var grid = $("#grid_DocumentLog").data("kendoGrid");
    for (var i = 0; i < grid.dataSource.view().length; i++) {
        var pItem = grid.dataSource.view()[i].items;
        for (var j = 0; j < pItem.length; j++) {
            var fItem = pItem[j].items;
            for (var k = 0; k < fItem.length; k++) {
                var rItem = fItem[k];
                documentDownloads.push({ ObjectTypeId: parseInt(rItem.ObjectTypeId), RecordId: parseInt(rItem.FormId) });
            }
        }
    }
    $.ajax({
        url: global.url.DownloadDocument,
        dataType: "json",
        type: "POST",
        contentType: 'application/json;charset=utf-8',
        data: JSON.stringify({ documentDownloads: documentDownloads }),
        async: true,
        cache: false,
        success: function () {
            var newWin = window.open('https://pmweb.hamad.qa/PMWebHelper/DocumentDownloader/DocumentDownloader.application', "_blank");
            if (!newWin || newWin.closed || typeof newWin.closed == 'undefined') {
                alert('Disable popup blocker to download.');
            }
...
function ShareGrid() {
    ...
    $.ajax({
        url: global.url.GetSearchShareLink,
        dataType: "html",
        type: "POST",
        contentType: 'application/json;charset=utf-8',
        data: JSON.stringify({ documentDownloads: documentDownloads }),
...
function getTabCount() {
    global.getAjax(global.url.GetTabCount, {}, (data) => {
        data = JSON.parse(data);
        $('.k-link', '#tabToUser').text('Assigned Documents (' + data.AssignedDocumentCount + ')');
        $('.k-link', '#tabWorkflowCC').text('My Workflow Email CC\'s (' + data.NotificationCount + ')');

        $('#spnTodayActionedCount').text(data.TodayActionedCount);
        if (parseInt(data.TodayActionedCount) > 0) {
            $('#btnActionedToday').removeClass('no-cursor');
            $('#btnActionedToday').click(function () {
                GetActionedDocuments(this, 1);
            });
        }

        $('#spnTotalNotification').text(data.NotificationCount);
        if (data.NotificationCount > 0) {
            $('#btnTotalNotification').removeClass('no-cursor');
            $('#btnTotalNotification').click(function () {
                GetEmailCCDocuments(this, 0);
            });
        }
        $('#spnTodayNotification').text(data.TodaysNotificationCount);
        if (data.TodaysNotificationCount > 0) {
            $('#btnTodayNotification').removeClass('no-cursor');
            $('#btnTodayNotification').click(function () {
                GetEmailCCDocuments(this, 1);
            });
        }

        $('#spnTotalAssigned').text(data.AssignedDocumentCount);
        if (data.AssignedDocumentCount > 0) {
            $('#btnTotalAssigned').removeClass('no-cursor');
            $('#btnTotalAssigned').click(function () {
                GetAssigned(0);
            });
        }
        $('#spnTodayAssigned').text(data.TodayAssignedDocumentCount);
        if (data.TodayAssignedDocumentCount > 0) {
            $('#btnTodayAssigned').removeClass('no-cursor');
            $('#btnTodayAssigned').click(function () {
                GetAssigned(1);
            });
        }
    });
...
var toUserDataId = 0;
function GetAssigned(dataId) {
    toUserDataId = dataId;
    $("#grid_ToUserDocuments").data("kendoGrid").dataSource.read();
}
function GetAssignedDocumentData() {
    var datepicker = $("#kendoDatePicker").data("kendoDatePicker");
    return {
        dataId: toUserDataId,
        dt: datepicker.value()
    };
