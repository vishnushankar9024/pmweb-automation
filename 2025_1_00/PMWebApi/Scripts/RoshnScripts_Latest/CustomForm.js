...
var CustomForm = {
    ...
    fromCompany: {
		init: (sender, args) => {
			var observer = new Roshn.MutationObserver((mutations, observer) => {
				...
				pjtMutations.forEach((m) => {
					setTimeout(CustomForm.fromCompany.bind(sender, args), 500);
        ...
        bind: function (sender, eventArgs) {
			var combo = $find($("[id$='ddlPHCompany']")[0].id);
			combo.add_itemsRequesting(function (sender, args) {
				args.set_cancel(true);
			});
			if(combo && combo.get_value() == 0){			
				if( $("[id$='ddlPHCompany']") && $("[id$='ddlPHCompany']").length > 0 ) {
					CustomForm.fromCompany.getCompany().then((company) => {
						if( company && company != null ) {
							...
							comboItem.set_text(company.CompanyName);
							comboItem.set_value(company.Id);
							combo.get_items().add(comboItem);
							combo.set_value(company.Id);
							combo.set_text(company.CompanyName);
							combo.commitChanges();
							return true;
						}
					});
