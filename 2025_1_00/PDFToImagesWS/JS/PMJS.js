    ...
    function popUpNote(isNew) {
        ...
        if (grid.get_masterTableView().get_selectedItems().length > 0) {
            if (!isNew) {
                ...
                window.open("DocumentNotesEditor.aspx",
                    'welcome', 'location=0,status=0,menubar=0,addressbar=0,resizable=1,scrollbars=1,width=1000,height=500,top=' + top + ',left=' + left)

                return false;
            }  
        }
        if (isNew) {
            return window.open("DocumentNotesEditor.aspx?IsNew=1",
                    'welcome', 'location=0,status=0,menubar=0,addressbar=0,resizable=1,scrollbars=1,width=1000,height=500,top=' + top + ',left=' + left);
...
function OnClientButtonClicking(sender, args) {
        ...
        function OpenHtmlEditorPopUp(ObjectType, RecordId, ColumnName, LiteralID) {
        
            var left = (screen.width - 600) / 2;
            var top = (screen.height - 500) / 2;
            window.open('HtmlNotes.aspx?ObjectType=' + ObjectType + '&RecordId=' + RecordId + '&ColumnName=' + ColumnName + '&LiteralID=' + LiteralID, null,
                     'location=0,status=0,menubar=0,addressbar=0,resizable=1,scrollbars=1,width=600,height=400,top=' + top + ',left=' + left);
...
/******************Paste from excel ***********************/
 ...
 function BindPasteExcelHandlers(rdgCtrl) {
        ...
        if (clipboard && CanPaste) {
            $('textarea[id$=txtClipboard]')[0].onpaste = function() { setTimeout("UpdateClipboardData()", 500); };
            $("[id$=" + rdgCtrl + "]").find(".rgCommandRow").focus(function() { $("#btnClipborad").click(); });
            $("[id$=" + rdgCtrl + "]").find(".rgCommandRow").click(function() { $("input[id$=btnClipborad]").click(); });
            $("input[id$=btnClipborad]").click(function() { $('textarea[id$=txtClipboard]').focus(); });
        }
    }
    ...
    function PasteValues() {
        ...
        $("[class^=SelectedToPaste]").each(function (cntrlIndxToPaste) {

            if (cntrlIndxToPaste > (arrPastedValues.length - 1)) currPasteValue = arrPastedValues[arrPastedValues.length - 1];
            if (i <= (arrPastedValues.length - 1)) {
                currPasteValue = arrPastedValues[i].replace(String.fromCharCode(13),'');
                i += 1;
            }
            //text box and radcombo
            if (((this.type == 'text') || (this.type == 'textarea')) && (this.className == 'SelectedToPaste')) {
                    ...
                    if (item) {
                        ...
                        if (combo._enableLoadOnDemand == true) {
                            ...
                            combo.add_itemsRequested(function (sender, eventArgs) {
                                var item = sender.findItemByText(currPasteValue);
                                if (item) {
                                    item.select();
                                    fireOnChange(this);
                                }
            ...
            // asp ddl
            if ((this.tagName == 'SELECT') && (this.className == 'SelectedToPaste')) {
    ...
    function SelectToPaste2(sender, eventArgs) {
        readyToPaste = true;
        $(".SelectedToPaste").each(function(Indx) { this.className = ''; });
        ...
        for (var i = 0; i < masterTable.get_dataItems().length; i++) {
           ...
           if (currCel != null){
               $(currCel).find('input[type=text][readOnly=false][disabled=false]').each(function(Indx) {
               if (!((this.id.match(/_dateInput$/)) || (this.id.match(/_dateInput_text$/)))){
                   this.className = 'SelectedToPaste'; 
                }                
                });
                $(currCel).find('select[disabled=false]').each(function(Indx) {
                    this.className = 'SelectedToPaste';
                });
                $(currCel).find('textarea[readOnly=false][disabled=false]').each(function(Indx) {
                    this.className = 'SelectedToPaste';
                });
        } 
    }
    ...
    function SelectToPaste(colIndx,sender) {
        ...
        for (var i = 0; i < objTds.length; i++) {
            var currTd = objTds[i];
            $(currTd).find('input[type=text][readOnly=false][disabled=false]').each(function(Indx) { this.className = 'SelectedToPaste'; });
            $(currTd).find('select[disabled=false]').each(function(Indx) { this.className = 'SelectedToPaste'; });
    ...
    function OpenNotificationLogPopup(LogId, recordDescription, type,Id,CustomTypeId,canedit,EntityId,EntityType) {
        ...
        window.open("NotificationLog.aspx?ObjectType=" + type + "&Id=" +
                 Id + "&LogId=" + LogId + "&RecordDescription=" + recordDescription + "&CustomFormTypeId=" + CustomTypeId + "&CanEdit=" + canedit + "&EntityId=" + EntityId + "&EntityType=" + EntityType, "Notification",
                'location=no,status=0,menubar=0,addressbar=0,resizable=1,scrollbars=1,width=1025,height=500,top=' + top + ',left=' + left);
        return false;
        //    }
    }
    ...
    function OpenRedliningMeasuresLogPopup(QuantityControlId, UOMControlId,UOMType, FloorId) {
        ...
        var win = window.open('RedliningMeasuresPopup.aspx?QuantityControlId=' + QuantityControlId + '&UOMControlId=' + UOMControlId + '&UOMType=' + UOMType + '&FloorId=' + FloorId, '',
                'location=no,status=0,menubar=0,addressbar=0,resizable=1,scrollbars=1,width=568,height=300,top=' + top + ',left=' + left);
        return false;
    }
    ...
    function OpenCompanyFilterPopup(ddlControlId,ddlId, Type,ddlProjectId) {
        ...
        if (ProjectId == '')
            ProjectId = 0;
        var win = window.open('CompaniesFilterPopup.aspx?ControlId=' + ddlControlId + '&ddlId=' + ddlId + '&Type=' + Type + '&ProjectRequired=1&ProjectId=' + ProjectId, '',
    ...
    function OpenCompanyFilterPopupProjectNotRequired(ddlControlId, ddlId, Type, ddlProjectId) {
        ...
        if (ProjectId == '')
            ProjectId = 0;
        var win = window.open('CompaniesFilterPopup.aspx?ControlId=' + ddlControlId + '&ddlId=' + ddlId + '&Type=' + Type + '&ProjectRequired=0&ProjectId=' + ProjectId, '',
                'location=no,status=0,menubar=0,addressbar=0,resizable=1,scrollbars=1,width=900,height=350,top=' + top + ',left=' + left);
        return false;
    }
    ...
    function OpenCompanyFilterPopupWithSource(ddlControlId, ddlId, Type, Source, ddlProjectId) {
        ...
        //if (ProjectId == '')
        //    ProjectId = 0;
        var win = window.open('CompaniesFilterPopup.aspx?ControlId=' + ddlControlId + '&ddlId=' + ddlId + '&Type=' + Type + '&Source=' + Source+'&ProjectRequired=0&ProjectId=0' , '',
                'location=no,status=0,menubar=0,addressbar=0,resizable=1,scrollbars=1,width=900,height=350,top=' + top + ',left=' + left);
        return false;
    }
...
/**********************/
    function NotificationCompletedChecked(sender,args) {
            ...
            if (chkIsCompleted.checked == true) {
              if(hddnNotificationTody!="")
              {    //var myDate = new Date(parseFloat(hddnNotificationTody.value));
        ...
        return false;
         ...
         function OpenTaskPopup() {
             OpenPOPUp('CheckListPopup.aspx', 775, 470, true, 'rdgtransmittlItems');
        return false;
    }
        ...
        if (ProjectId == '')
            ProjectId = 0;
        var win = window.open('CompaniesFilterPopup.aspx?hdnIds=' + hdnIds + '&ddlId=' + ddlId + '&Type=' + Type + '&ProjectRequired=1&ProjectId=' + ProjectId + '&hdnNames=' + hdnNames + '&ddlType=Multiple', '',
                'location=no,status=0,menubar=0,addressbar=0,resizable=1,scrollbars=1,width=900,height=350,top=' + top + ',left=' + left);
        return false;
    }
      ...
      function OpenNotificationSingleCompanyFilterPopup(txtFullContact,txtContact,txtEmail,txtIds, Type,Source) {
        ...
        var win = window.open('CompaniesFilterPopup.aspx?txtFullContact=' + txtFullContact + '&txtContact=' + txtContact + '&txtEmail=' + txtEmail + '&Type=' + Type + '&txtIds=' + txtIds + '&Source=' + Source + '&ProjectRequired=0&ProjectId=' + ProjectId, '',
                'location=no,status=0,menubar=0,addressbar=0,resizable=1,scrollbars=1,width=900,height=350,top=' + top + ',left=' + left);
        return false;
      }
   ...
   function  chkCheckListCompletedChecked(sender,args) {
           var hddnToday = $("[id$=hdnCheckListTodayDate]")[0];
            ...
            if (chkIsCompleted.checked == true) {
              if(hddnToday.value!="")
              {    //var myDate = new Date(parseFloat(hddnToday.value));
     ...
     function OpenNoteDetailPopup(txtNoteId) {
        ...
        var win = window.open('NotesPopup.aspx?txtNotesId=' + txtNoteId, '',
     ...
     function OpenScoringNoteDetailPopup(txtNoteId, Option) {
         var left = (screen.width - 620) / 2;
         var top = (screen.height - 320) / 2;
         var win = window.open('NotesPopup.aspx?txtNotesId=' + txtNoteId + '&Option=' + Option + '&Source=Scoring' , '',
                 'location=no,status=0,menubar=0,addressbar=0,resizable=1,scrollbars=1,width=620,height=400,top=' + top + ',left=' + left);
         return false;
     }

    function OpenViewNoteDetailPopup(txtNoteId) {
        var left = (screen.width - 620) / 2;
        var top = (screen.height - 320) / 2;
    ...
    function OpenGoogleLinearPicker(ObjectType, ObjectId, ComponentId, PickerSender) {
        var left = (screen.width - 900) / 2;
        var top = (screen.height - 600) / 2;
        window.open("GoogleAddressesPicker.aspx?ShowLinearTab=true&RecordType=" + ObjectType + "&ObjectId=" + ObjectId + "&ComponentId=" + ComponentId + "&PickerSender=" + PickerSender,
            'welcome', 'location=0,status=0,menubar=1,addressbar=0,resizable=1,scrollbars=1,width=900,height=600,top=' + top + ',left=' + left);
        return false;
    }
    ...
    function OpenWBSPopup(ddlWBS,ProjectId,ProgramId) {
        ...
        var win = window.open('WBSPopup.aspx?ddlWBS=' + ddlWBS +  '&ProjectId=' + ProjectId +'&ProgramId='+ProgramId, '',
    ...
    function OpenVisualCalculator(Id) {
        var left = (screen.width - 883) / 2;
        var top = (screen.height - 594) / 2;
        var win = window.open('VisualCalculatorPopup.aspx?Id=' + Id, '',
                'location=no,status=0,menubar=0,addressbar=0,resizable=1,scrollbars=1,width=1024,height=594,top=' + top + ',left=' + left);
        return false;
    ...
    function OpenBarCodePopup(fldBarcode, fldFormat, ObjectType, Id) {
        ...
        OpenPOPUp('BarCodePopup.aspx?fldBarcode=' + fldBarcode + '&fldFormat=' + fldFormat + '&barcode=' + barcode + '&format=' + format + '&ObjectType=' + ObjectType + '&RecordId=' + Id, 500, 400, false);
        return false;
    }
    ...
    function OpenConversionRatePopup(ObjectType,ObjectId) {
        OpenPOPUpToRedirect('CurrencyPopup.aspx?ObjectType=' + ObjectType + '&ObjectId=' + ObjectId, 480, 550, false);
        return false;
    }

    function OpenDefaultCurrencyPopup(EntityId, EntityType) {
        OpenPOPUpToRedirect('DefaultCurrencyPopup.aspx?EntityId=' + EntityId + '&EntityType=' + EntityType, 450, 550, false);
        return false;
    }


    function OpenBarCodeSettingsPopup(fldfield, fldformat,IsAsset) {
        OpenPOPUp('BarCodeSettingsPopup.aspx?fldfield=' + fldfield + '&fldFormat=' + fldformat + '&IsAsset=' + IsAsset , 400, 200, false);
        return false;
    }
    ...
    function OpenOverageDetailFromItem(Id, ActualAmount, OverageAmount, Save,OverageId) {
        ...
        var left = (screen.width - 415) / 2;
        var top = (screen.height - 425) / 2;
        var win = window.open('OverageDetailsPopup.aspx?Id=' + Id + '&ActualAmount=' + ActualAmount + '&OverageAmount=' + OverageAmount +'&OverageId='+ OverageId+ '&Save=' + Save, '',
                'location=no,status=0,menubar=0,addressbar=0,resizable=1,scrollbars=1,width=415,height=425,top=' + top + ',left=' + left);
        return false;
    }
    function OpenSelectLeasePopupFromTenantInvoices(Id, ddlId, hdnId) {
         OpenPOPUp('SelectLeasePopup.aspx?Id=' + Id + '&ddlId=' + ddlId + '&hdnId=' + hdnId + '&Source=Tenant', 100, 100, false);
        return false;
    }
    ...
    function OpenSelectLeasePopupFromLease(Id, ddlId) {
        OpenPOPUp('SelectLeasePopup.aspx?Id=' + Id + '&ddlId=' + ddlId + '&Source=Lease', 100, 100, false);
        return false;
    }
    function OpenEscalationDetailPopup(Id, txtCurrentAmountId,txtNewAmountId, LeaseId) {
        ...
        var top = (screen.height - 500) / 2;
        var win = window.open('EscalationDetailsPopup.aspx?Id=' + Id + '&txtNewAmountId=' + txtNewAmountId + '&CurrentAmount=' + CurrentAmount + '&LeaseId=' + LeaseId + '&RecordId=0' , '',
                'location=no,status=0,menubar=0,addressbar=0,resizable=1,scrollbars=1,width=435,height=500,top=' + top + ',left=' + left);
    }

    function OpenEscalationDetailItemPopup(Id, CurrentAmount, LeaseId,RecordId) {
        var left = (screen.width - 435) / 2;
        var top = (screen.height - 500) / 2;
        var win = window.open('EscalationDetailsPopup.aspx?Id=' + Id  + '&CurrentAmount=' + CurrentAmount + '&LeaseId=' + LeaseId + '&RecordId=' + RecordId, '',
                'location=no,status=0,menubar=0,addressbar=0,resizable=1,scrollbars=1,width=435,height=500,top=' + top + ',left=' + left);
        return false;
    }
    ...
    function OpenNotificationPopup(RecordType, RecordId, RecordDescription, EntityId, EntityTypeId, CustomFormTypeId) {
        var left = (screen.width - 900) / 2;
        var top = (screen.height - 500) / 2;
        window.open("Notification.aspx?ObjectType=" + RecordType + "&Id=" +
                 RecordId + "&Description="
                            ...
                            + "&CustomFormTypeId=" + CustomFormTypeId, "Notification",
    ...
    function MainDropDown_SelectedIndexChanging(sender, eventArgs) {
        ...
        if (parseInt(RecordId) > 0) {
            ...
            if (currPageId == 7 || currPageId == 181) {
                window.location.href = currPageName + '?ItemId=' + RecordId ;
            }
            else{
