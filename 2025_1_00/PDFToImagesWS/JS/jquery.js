...
//http: //echonull.colinta.com/?p=6
jQuery.fn.changeCheckbox = function(a) { $(this).keyup(function() { if (e.which == 32) { $(this).attr('checked', !$(this).attr('checked')).click(); return false } }); $(this).click(a) };
...
//
jQuery.fn.LongtitudeText = function (options, callback) {
    var allowNegative = true;
    var decimalCharacter = ".";
    var isInt = (decimalCharacter == "");
    this.keypress(function (e) {
        ...
        if (key < 48 || key > 57) {
            ...
            if (allowNegative && key == 45 && (($(this).getSelection().start == 0 && $(this).getSelection().end == $(this).val().length) || this.value.indexOf("-") == -1)) return true;
            /* only one decimal separator allowed */
            if (key == decimalCharacter.charCodeAt(0) && this.value.indexOf(decimalCharacter) != -1) {
                allow = false;
            }
            ...
            if (key == 44 & this.value.indexOf(decimalCharacter) == -1) {
                if (document.selection) {
            ...
            if (key == decimalCharacter.charCodeAt(0) && this.value.indexOf(decimalCharacter) == -1) {
                if (isInt && e.which > 0) { return false; }
                allow = true;
            }
        }
        else {
		    ...
		    function (e) {
		        ...
		        if (Res.length > 0) {
		            ...
		            for (var i = 0; i < x.length; i++) {
		                if (i == 0 && x.charAt(i) == '-') {
		                    nbrbeforedec = x.charAt(i);
		                }
		                else if (x.charAt(i) != '-') {
		                    if (total < 5) {
		                        nbrbeforedec = nbrbeforedec + x.charAt(i);
		                        total = total + 1;
		                    }
		                }
		            }
		        ...
		        if (Res.length >= 2) {
		            if (Res[1] != '' && Res[1] != null) {
		                Res[1] = Res[1].replace('-', '');
		                nbrbeforedec = nbrbeforedec + '.' + Res[1];

		            }
		        }
		        var lastRes = nbrbeforedec.split('.');
		        var isnumeric = true;
		        if (lastRes.length > 0) {
		            if (isNumLong(lastRes[0]) == false) {
		                isnumeric = false;
		            }
		        }
		        if (lastRes.length >= 2) {
		            if (isNumLong(lastRes[1]) == false) {
		                isnumeric = false;
		            }
		        }
		        if (isnumeric == false) {
...
jQuery.fn.numeric = function (options, callback) {
    ...
    var isInt = (decimalCharacter == "");
    var MaxAttr = $(this).attr("maxnumber");
    var MinAttr = $(this).attr("minnumber");
    var PrecisionAttr = $(this).attr("precision");
    var DecimalCharAttr = $(this).attr("decimalChar");
    var SeparatorAttr = $(this).attr("separator");
    var decimalPrecision = options.DecimalPrecision || (isInt ? "0" : "2");
    if (isInt == false && $(this).attr("precision") != null) {
        decimalPrecision = $(this).attr("precision")
    }
    ...
    this.focus(
            function (e) {
                ...
                if (this.readOnly != "readOnly" && this.readOnly != "true" && this.readOnly != true) {
                    $(this).val(numberFormatter.RemoveChars($(this).val(), decimalCharacter, decimalSeparator));
	    ...
	    .blur(
		    function (e) {
		        ...
		        if (this.readOnly != "readOnly" && this.readOnly != "true" && this.readOnly != true) {
		            ...
		            var newVal;
		            if (isInt) {
		                newVal = allowNegative ? CInt(val) : CPInt(val);
		            } else {
		                PrecisionAttr = $(this).attr("precision");
		                newVal = allowNegative ? CDbl(val, decimalCharacter, decimalSeparator, PrecisionAttr) : CPDbl(val, decimalCharacter, decimalSeparator, PrecisionAttr);
		            }
		            if (isNaN(newVal)) {
		                newVal = "0";
		            }
		            if (!isInt) {
		                PrecisionAttr = $(this).attr("precision");
		                DecimalCharAttr = $(this).attr("decimalChar");
		                MaxAttr = $(this).attr("maxnumber");
		                MinAttr = $(this).attr("minnumber");
		                SeparatorAttr = $(this).attr("separator");
		                decimalPrecision = options.DecimalPrecision || (isInt ? "0" : "2");

		                newVal = FMM(newVal, MinAttr, MaxAttr);

		                newVal = FPrec(newVal.toString(), PrecisionAttr, DecimalCharAttr, SeparatorAttr);
		                jQuery(this).val(newVal);
		            } else {
		                jQuery(this).val(FMM(newVal, MinAttr, MaxAttr));
		            }
		        }
		    }
	    );
