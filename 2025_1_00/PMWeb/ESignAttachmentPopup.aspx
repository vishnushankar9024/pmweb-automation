...
<head runat="server">
    ...
    <telerik:RadCodeBlock ID="CodeBlock" runat="server">
        <script type="text/javascript">
            ...
            function OpenPOPUpToRefreshByHref(URL, Width, Height, windowName) {
                let left = (window.innerWidth - Width) / 2;
                let top = (window.innerHeight - Height) / 2;
                const features =
                    'location=0,status=0,menubar=0,resizable=1,scrollbars=1,' +
                    `width=${Width},height=${Height},top=${top},left=${left}`;

                window.parent.ESignExternalPopup = window.parent.open(
                    URL,
                    `${windowName}`,
                    features
