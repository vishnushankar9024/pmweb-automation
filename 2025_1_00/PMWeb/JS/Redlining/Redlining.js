...
var txtTextId = '';
var clickNumber = 0; var clicks = 0;
var intThickness = 2;
...
var skin; var originalAnnCount = 0; var originalMsrCount = 0;
var undoIndex = 0;
var redoIndex = 1;
var paper;
var canvas;
...
var tooltip = null;
var polylineMSR;
var imgANNHeight = '', imgANNWidth = '', imgMSRHeight = '', imgMSRWidth = '';
var PMWebViewerPanelPosition = 0; var PMWebViewerRightPaneWidth = 0; var PMWebViewerRightPaneHeight = 0;


function OpenViewerNotesPopup(Src) {
    var browserWidth = $telerik.$(window).width();
    var browserHeight = $telerik.$(window).height();
    var wnd = window.radopen('PMWebViewerNotePopup.aspx?Source=' + Src);
    if (isMobileScreen()) {
        wnd.setSize(browserWidth - 10, browserHeight);
        wnd.moveTo(0, 0);
    } else {
        var wnWidth = (browserWidth * 0.3 > 500) ? browserWidth * 0.3 : 500;
        var wnHeight = browserHeight * 0.9;
...
function OpenMenu(me, x1, y1) {
    ...
    if (drawingsCateg == 'ANN') {
        ...
        for (var i = 0; i < arrMeasures.length; i++) {
            if (arrMeasures[i].TransformID == elemClicked) { value = arrMeasures[i].CENTROID }
        }

        if (value == '')
        { contextMenu.findItemByValue('EditText').set_enabled(false); }
        else
        { contextMenu.findItemByValue('EditText').set_enabled(true); }
    }
    if (isEdit) {
        contextMenu.findItemByValue('EndEdit').set_enabled(true);
        contextMenu.findItemByValue('Delete').set_enabled(false);
    }
    else {
        contextMenu.findItemByValue('EndEdit').set_enabled(false);
        contextMenu.findItemByValue('Delete').set_enabled(true);
    }
...
function ReDrawANN() {
    ...
    if (drawingsCateg == "ANN") {
        ...
        if (document.getElementById(imgCanvasId).style.width == '0px') {
            document.getElementById(imgCanvasId).style.width = imgANNWidth;
            document.getElementById(imgCanvasId).style.height = imgANNHeight;
    }
    }
    else if (drawingsCateg == "MSR") {
        $('#Canvas')[0].innerHTML = $('#Canvas')[0].children['imgCanvas'].outerHTML + $('#Canvas')[0].children['divResize'].outerHTML;
        if (document.getElementById(imgCanvasId).style.width == '0px') {
            document.getElementById(imgCanvasId).style.width = imgMSRHeight;
            document.getElementById(imgCanvasId).style.height = imgMSRHeight;
        }
    }
        ...
        if (isVisible == true) {
            switch (tempType) {
                ...
                case 'SPACE':
                    ...
                    var img = paper.image("Images/Redlining/smallSpace" + skin + ".png", tempX1 * intZoomMultiplier, tempY1 * intZoomMultiplier, 16, 16)
                ...
                case 'EQUIPMENT':
                    ...
                    var img = paper.image("Images/Redlining/smallEquipment" + skin + ".png", tempX1 * intZoomMultiplier, tempY1 * intZoomMultiplier, 16, 16)
                    img.id = equipmentId
                ...
                default:
                    ...
                    if (arrDrawings[i].MAINPAGE) {
                        ellipse.click(function () { OpenLinkToolTip(this); });
                        text.click(function () { OpenLinkToolTip(this); });
                    }
            ...
            if (drawingsCateg == 'ANN') {
                ...
                for (var i = 0; i < arrMeasures.length; i++) {
                    if (arrMeasures[i].TransformID == elemClicked) {
                        if (MSR_Set) {
                            if (Me.inlineTextEditing.input.value == '') MSR_Set.pop(Me);
                        }
...
function CreateToolTip(me) {
    ...
    if (!div) {
        if (me.id.indexOf('linkedrecord_') == 0) {
            var divX = mouseX;
            var divY = mouseY;
        } else {
            var divX = mouseX + $('#divZoomer').scrollLeft();
            var divY = mouseY + $('#divZoomer').scrollTop();
        }
        div = document.createElement("div");
        div.id = me.id;
        div.setAttribute("VALUE", me.attrs.VALUE);
        div.style.left = divX + 'px'; //parseInt(me.attrs.x) + 'px';
        div.style.top = divY + 'px';// parseInt(me.attrs.y) + 'px';
        div.style.position = "absolute";
        $('#Canvas')[0].appendChild(div);
        showToolTip(div);
...
function ReDrawMSR() {
    ...
    for (var i = 0; i < arrMeasures.length; i++) {
        ...
        intThickness = arrMeasures[i].STROKE + 1;
        ...
        switch (tempType) {
            case 'polygon':
                ...
                if (clickNumber >= 2) {
                    ...
                    arrMeasures.push({ 'TYPE': 'POLYGON', 'ID': '', 'CENTROID': '', 'LENGTH': 0, 'AREA': tmpArea, 'ARRPOLYGONPOINTS': arrPolygonPoints, 'COLOR': polyColor, 'STROKE': polyStroke - 1, 'USER': '', 'DATE': '', 'VISIBLE': true, 'TransformID': 'polyline' + countId });
                    arrXPoints = [];
                    arrYPoints = [];
                    for (var j = 0; j < arrPolygonPoints.length; j++) {
                        arrXPoints.push(arrPolygonPoints[j].X * intZoomMultiplier);
...
function SaveDrawings(sender) {
    ...
    PageMethods.SaveDrawings(arrayAll,
            function (response) { $("[id$=btnHiddenSave]").click(); },
...
function HandleThicknessChanged(sender, eventArgs) {
    ...
    intThickness = sender.get_value() + 1;
...
function LoadThickness(sender, eventArgs) {
    var circleWidth = sender.get_value();
    intThickness = sender.get_value() +1;
    var imgTich = document.getElementById('imgThickness');
    var lblTich = document.getElementById('lblThickness');
...
function HandleZoomChanged(sender, eventArgs) {
    ...
    if (drawingsCateg == 'ANN') {
        var txtZoomValue = document.getElementById('ctl00_CPH1_PMWebViewerDrawing1_mainToolBar_i15_i0_txtZoomValue');
    }
    else {
    ...
    if (drawingsCateg == 'ANN') {
        ...
        if(imgANNHeight == '')
        {
            imgANNHeight = document.getElementById(imgCanvasId).style.height;
            imgANNWidth = document.getElementById(imgCanvasId).style.width;
    }
    }
    else {
        hdnZoom = $("[id$=hdnZoomMsr]");
        if(imgMSRHeight == '') {
            imgMSRHeight = document.getElementById(imgCanvasId).style.height;
            imgMSRWidth = document.getElementById(imgCanvasId).style.width;
            }
    }
    if (hdnZoom)
    ...
    if (drawingsCateg == "ANN") {
        ...
        if (toolbar) {
            toolbar.get_items().getItem(8).set_text(sender.get_value() + '%')
        }
    }
...
function RedliningOnClientButtonClicking(sender, args) {
    ...
    var toolbar;
    if (drawingsCateg == 'ANN') {
        contextMenu = $find("ctl00_CPH1_PMWebViewerDrawing1_cmDrawing");
        toolbar = $find("ctl00_CPH1_PMWebViewerDrawing1_mainToolBar");
    }
    else {
        contextMenu = $find("ctl00_CPH1_PMWebViewerMeasurement1_cmDrawing");
        toolbar = $find("ctl00_CPH1_PMWebViewerMeasurement1_mainToolBar");
    }
    ...
    switch (args.get_item().get_commandName()) {
        case 'line':
            ...
            clickNumber = 0;
            tempType = 'line';
            var a = toolbar.findItemByAttribute('key', 'Lines');
            a.get_linkElement().className = "Toolbarline";
            if (drawingsCateg == 'MSR')
            { UpdateLastSelectedTool(tempType); }
            break;
        ...
        case 'polyline':
            ...
            var a = toolbar.findItemByAttribute('key', 'Lines');
            a.get_linkElement().className = "Toolbarpolyline";
        ...
        case 'rectangle':
            ...
            $('#Canvas').hover(function () { $(this).css('cursor', 'pointer') })
            cursorType = 'Draw'
            var a = toolbar.findItemByAttribute('key', 'Shapes')
            a.get_linkElement().className = "Toolbarrectangle"
            if (drawingsCateg == 'MSR')
            { UpdateLastSelectedTool(tempType); }
            break;
        ...
        case 'ellipse':
            ...
            cursorType = 'Draw'
            var a = toolbar.findItemByAttribute('key', 'Shapes')
            a.get_linkElement().className = "Toolbarellipse"
            if (drawingsCateg == 'MSR')
            { UpdateLastSelectedTool(tempType); }
            break;
        ...
        case 'text':
            ...
            $('#Canvas').hover(function () { $(this).css('cursor', 'pointer') })
            cursorType = 'Draw'
            var a = toolbar.findItemByAttribute('key', 'Docs')
            break;
        ...
        case 'bubble':
            ...
            cursorType = 'Draw'
            var a = toolbar.findItemByAttribute('key', 'Shapes')
            a.get_linkElement().className = "ToolbarBubble"
            if (drawingsCateg == 'MSR')
            { UpdateLastSelectedTool(tempType); }
            break;
        ...
        case 'Undo':
            ...
            if (drawingsCateg == 'ANN') {
                if (arrDrawings.length > 0) {
                    ...
                    ReDrawANN();
                    deletedId = poppedDrawing.ID;

                    if (originalAnnCount >= arrDrawings.length)
                    { EnableToolBarButton(undoIndex, false); }
                    else { EnableToolBarButton(undoIndex, true); }

                    if (deletedId != '' && isNumeric(deletedId)) {
                        AutoDeleteDrawings(deletedId, drawingsCateg);
                        if (autoSave == 'True') { __doPostBack('ctl00$CPH1$PMWebViewerDrawing1$rdgActions$ctl00$ctl02$ctl00$btnRefresh', ''); }
                    }
            ...
            if (drawingsCateg == 'MSR') {
                if (arrMeasures.length > 0) {
                    ...
                    EnableToolBarButton(redoIndex, true);
                    ReDrawMSR();
                    deletedId = poppedMeasure.ID;
                    if (originalMsrCount >= arrMeasures.length)
                    { EnableToolBarButton(undoIndex, false); }
                    else { EnableToolBarButton(undoIndex, true); }
        ...
        case 'Redo':
            ...
            if (drawingsCateg == 'ANN') {
                ...
                arrDrawings[arrDrawings.length - 1].ID = '';
                if (autoSave == 'True') {
                    AutoSaveDrawings()
                }
                if (arrDeletedDrawings.length > 0) {
            ...
            if (drawingsCateg == 'MSR') {
                ...
                arrMeasures[arrMeasures.length - 1].ID = '';
                if (autoSave == 'True') {
                    AutoSaveDrawings()
                }
                if (arrDeletedMeasures.length > 0) {
        ...
        case 'LinkRecords':
            ...
            var browserWidth = $telerik.$(window).width();
            var browserHeight = $telerik.$(window).height();
            var wnd = OpenPOPUp('GlobalLinkedRecords.aspx?Id=' + RecordId + '&ProjectId=' + ProjectId + '&Source=' + Source);          
            $('#Canvas').hover(function () { $(this).css('cursor', 'pointer') })
            cursorType = 'Draw'
            break;
        ...
        case 'Default':
            ...
            cursorType = 'Default'
            tempType = '';
            var a = toolbar.findItemByAttribute('key', 'Mouse')
            a.get_linkElement().className = "ToolbarDefault"
            break;
        case 'Move':
            if ($telerik.isFirefox) {
                $('#Canvas').hover(function () { $(this).css('cursor', '-moz-grab') })
            }
            else if ($telerik.isChrome) {
                $('#Canvas').hover(function () { $(this).css('cursor', '-webkit-grab') })
            }
            else if ($telerik.isIE9 || $telerik.isIE10) {
                $('#Canvas').hover(function () { $(this).css('cursor', 'grab') })
            }
            else {
            ...
            cursorType = 'Move'
            var a = toolbar.findItemByAttribute('key', 'Mouse')
            a.get_linkElement().className = "ToolbarMove"
            break;
...
function OnclientExpanded(sender) {
...
function UpdatePanelSettings(sender) {
    $.ajax({
        ...
        data: JSON.stringify({ 'isPinned': sender.get_docked(), Width: sender.get_width(), PMWebViewerRightPaneWidth: PMWebViewerRightPaneWidth, PMWebViewerRightPaneHeight: PMWebViewerRightPaneHeight }),
        dataType: "json",
        async: true
    });
...
function chkActionChange(sender) {
    ...
    rdgActions.find("input[id$='chkVisible']").each(function () {
        if (sender.checked) {
            if (!this.disabled && !this.checked) isChecked = false;
        }
        i++;
    });
    if (!sender.checked) {
...
function AddLinkedRecordFromMenu(LRId, LRCode, LRType, LRTypeId, LRNumber) {
    ...
    if (tempType == 'LinkedRecord') {
        ...
        EnableToolBarButton(undoIndex, true);
        arrDrawings.push({ 'TYPE': 'SPACE', 'ID': '', 'VALUE': tempStamp, 'X': menuX / intZoomMultiplier, 'Y': menuY / intZoomMultiplier, 'SPACEID': spaceId, 'RANDOM': '', 'COLOR': currColor, 'STROKE': intThickness - 1, 'USER': '', 'DATE': '', 'VISIBLE': true, 'TRANSFORM': '', 'TransformID': 'SPACE_' + spaceId + '_' + strRnd });
        var img = paper.image("Images/Redlining/smallSpace" + skin + ".png", menuX, menuY, 16, 16)
        ...
        EnableToolBarButton(undoIndex, true);
        arrDrawings.push({ 'TYPE': 'EQUIPMENT', 'ID': '', 'VALUE': tempStamp, 'X': menuX / intZoomMultiplier, 'Y': menuY / intZoomMultiplier, 'EQUIPMENTID': equipmentId, 'COLOR': currColor, 'STROKE': intThickness - 1, 'USER': '', 'DATE': '', 'VISIBLE': true, 'TRANSFORM': '', 'TransformID': 'EQU_' + countId });

        var img = paper.image("Images/Redlining/smallEquipment" + skin + ".png", menuX, menuY, 16, 16)
        img.id = 'EQU_' + countId;
        var text = paper.text(menuX + 18, menuY + 7, tempStamp).attr(
             {
                 "text-anchor": "start",
...
function addText(x, y) {
    EnableToolBarButton(undoIndex, true);
    arrDrawings.push({ 'TYPE': 'TEXT', 'ID': '', 'TRANSFORM': '', 'VALUE': '', 'X': x / intZoomMultiplier, 'Y': y / intZoomMultiplier, 'FONT': '', 'COLOR': currColor, 'STROKE': intThickness - 1, 'USER': '', 'DATE': '', 'VISIBLE': true, 'TransformID': 'text' + countId });
    var fontsize = 12 * intZoomMultiplier
    var text = paper.text(x, y + 7, '').attr({
        stroke: 'none',
        fill: currColor,
        "stroke-width": intThickness,
        font: fontsize + 'px "verdana,geneva,helvetica,sans-serif"',
        "font-family": "verdana,geneva,helvetica,sans-serif",
        "font-weight": "normal",
        "text-anchor": "start"
...
function cmDrawingClicked(sender, e) {
    switch (e.get_item().get_value()) {
        ...
        case 'AddNote':
            var hdnNotes = $("[id$=hdnNotes]")[0];
            hdnNotes.value = '';
            OpenViewerNotesPopup('Menu');
            break;
        case 'AddLinkedRecord':
            var browserWidth = $telerik.$(window).width();
            var browserHeight = $telerik.$(window).height();
            var wnd = OpenPOPUp('GlobalLinkedRecords.aspx?Id=' + RecordId + '&ProjectId=' + ProjectId + '&Source=PMWebViewer' + '&From=Menu');          
            break;
        case 'AddImgStamp':
            var browserWidth = $telerik.$(window).width();
            var browserHeight = $telerik.$(window).height();
...
function AddStampFromMenu() {
    ...
    arrDrawings.push({ 'TYPE': 'IMAGESTAMP', 'ID': '', 'TRANSFORM': '', 'VALUE': strval, 'X': menuX / intZoomMultiplier, 'Y': menuY / intZoomMultiplier, 'COLOR': currColor, 'STROKE': intThickness - 1, 'USER': '', 'DATE': '', 'VISIBLE': true, 'TransformID': 'imgstamp' + countId });
    EnableToolBarButton(undoIndex, true);
...
function stampMenuItemClicked(value) {
    EnableToolBarButton(undoIndex, true);
    arrDrawings.push({ 'TYPE': 'STAMP', 'ID': '', 'TRANSFORM': '', 'VALUE': value, 'X': menuX / intZoomMultiplier, 'Y': menuY / intZoomMultiplier, 'COLOR': currColor, 'STROKE': intThickness - 1, 'USER': '', 'DATE': '', 'VISIBLE': true, 'TransformID': 'stamp' + countId });
    var scaledSize = 14 * intZoomMultiplier;
    ...
    if (CanEdit == 'True') {
        if (isEdit == '') {
            ...
            if (ft != null) {
                ...
                if (strTrans != '' && strTrans && (strTrans.indexOf('m') == -1 || strTrans.indexOf('m') > 0)) {
    ...
    switch (e.get_item().get_value()) {
        ...
        case 'Delete':
            if (drawingsCateg == "ANN") {
                for (var i = 0; i < arrDrawings.length; i++) {
                    if (arrDrawings[i].TransformID == elemClicked) {
                        var id = arrDrawings[i].ID.replace("NOTE_", "")
                        if (arrDrawings[i].ID != '' && isNumeric(id)) {
                            AutoDeleteDrawings(id, drawingsCateg);
                            if (autoSave == 'True')
            ...
            if (drawingsCateg == "MSR") {
                for (var i = 0; i < arrMeasures.length; i++) {
                    if (arrMeasures[i].TransformID == elemClicked) {
                        if (arrMeasures[i].ID != '') {
                            AutoDeleteDrawings(arrMeasures[i].ID, drawingsCateg);
...
function OnMenuShowing(sender,event) {
    menuX = mouseX
    menuY = mouseY
    if (cursorType == 'Move') {
        cursorType = 'Default';
...
function resetZoom() {
    ...
    if (drawingsCateg == 'ANN')
    { sldrZoom = $find("ctl00_CPH1_PMWebViewerDrawing1_mainToolBar_i15_i0_sldrZoom"); }
    else
    { sldrZoom = $find('ctl00_CPH1_PMWebViewerMeasurement1_mainToolBar_i16_i0_sldrZoom'); }
...
function RecalculateSavedMsr() {
    var txtGrd = $('#ctl00_CPH1_PMWebViewerMeasurement1_txtDisGrd').val()
    var txtMap = $('#ctl00_CPH1_PMWebViewerMeasurement1_txtDisMap').val()
    var newScaleMultiplier = txtGrd / txtMap;
    ...
    for (var i = 0; i < arrMeasures.length; i++) {
        ...
        switch (tempType) {
            case 'LINE':
                ...
                tempX2 = arrMeasures[i].X2;
                tempY2 = arrMeasures[i].Y2;
                var tempLength = LengthCalculation(tempX1, tempY1, tempX2, tempY2);
                arrMeasures[i].LENGTH = tempLength;
                break;
            case 'POLYGON':

                arrPolygonPoints = arrMeasures[i].ARRPOLYGONPOINTS;
                var tmpArea = PolygonAreaCalculation(arrPolygonPoints);
                arrMeasures[i].AREA = tmpArea;
                break;
            ...
            case 'RECTANGLE':
                ...
                tempY2 = arrMeasures[i].Y2;
                var arrTempPoints = [];
                arrTempPoints.push({ X: tempX1, Y: tempY1 });
                arrTempPoints.push({ X: (tempX1 + tempX2), Y: tempY1 });
                arrTempPoints.push({ X: (tempX1 + tempX2), Y: (tempY1 + tempY2) });
                arrTempPoints.push({ X: tempX1, Y: (tempY1 + tempY2) });
            ...
            case 'ELLIPSE':
                ...
                tempX2 = arrMeasures[i].X2;
                tempY2 = arrMeasures[i].Y2;
                var tmpArea = EllipseAreaCalculation(tempX2, tempY2);
                arrMeasures[i].AREA = tmpArea;
                break;
        }

    }
    PageMethods.UpdateMeasurements(arrMeasures, newScaleMultiplier);
}

function toolbarClick(commandName) {
    if (drawingsCateg == 'MSR') {
        if (commandName == 'polygon')
        { commandName = 'polyline' }
        var toolbar = $find("ctl00_CPH1_PMWebViewerMeasurement1_mainToolBar");
        toolbar.findButtonByCommandName(commandName).click()
    }
}