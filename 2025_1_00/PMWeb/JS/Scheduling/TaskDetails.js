...
Grids.OnDataReceive = function (G, Source) {
    if (Source.Name == "Data") {
        ...
        if (TaskCount > 3000) {
            G.Cols.G.GanttSlack = 0;
            G.Cols.G.GanttPaging = 1;
            G.Cols.G.GanttPagingFixed = 7;
            G.Cols.G.GanttPageWidth = 1000;
            G.Cols.G.GanttPageInit = 0;
            G.Cols.G.GanttAllDependencies = 5;
            G.Cols.G.GanttMaxDependencies = 20;
            G.Cols.G.GanttAdjacentDependencies = 2;
            G.Toolbar.ZoomOutVisibleFormula = 'return false';
            G.Toolbar.ZoomInVisibleFormula = 'return false';
            G.Toolbar.ZoomFitVisibleFormula = 'return false';
            G.Actions.OnDragGanttHeader = "";
            G.Actions.OnClickGanttHeader = "";
            G.Actions.OnRightClickGanttHeader = "";
        }
    }
}
...
Grids.OnSave = function (G, row) {
    ...
    while (Row != null) {
        if ( (Get(Row, 'S') != '' && (Get(Row, 'S') >= 4102444800000 || Get(Row, 'S') < 0))  // 4102444800000 represent 1/1/2100
         || (Get(Row, 'E') != '' && (Get(Row, 'E') >= 4102444800000 || Get(Row, 'E') < 0))
         || (Get(Row, 'BS') != '' && (Get(Row, 'BS') >= 4102444800000 || Get(Row, 'BS') < 0))
         || (Get(Row, 'BF') != '' && (Get(Row, 'BF') >= 4102444800000 || Get(Row, 'BF') < 0))
         || (Get(Row, 'AS') != '' && (Get(Row, 'AS') >= 4102444800000 || Get(Row, 'AS') < 0))
         || (Get(Row, 'AF') != '' && (Get(Row, 'AF') >= 4102444800000 || Get(Row, 'AF') < 0))) {
            alert(Msg_DateOutOfRange + Get(Row, 'id') + ')')
            i = i + 1;
        }
        Row = G.GetNext(Row);
    }
    ...
    $.ajax({
        ...
        data: JSON.stringify({ GanttBase: GanttBase, GanttFinish: GanttFinish }),
        dataType: "json",
        async: true
    });
}
...
function SaveGroupAndPageLength(GroupCols, PageLength, RefreshGantt) {
    ...
    $.ajax({
        ...
        contentType: "application/json; charset=utf-8",
        data: JSON.stringify({ GroupCols: GroupCols, PageLength: PageLength }),
        dataType: "json",
        async: true
    });
...
function PageLengthChanged(val) {
    ...
    $.ajax({
        ...
        contentType: "application/json; charset=utf-8",
        data: JSON.stringify({ PageLength: val }),
        dataType: "json",
        async: true
    });
...
function ShowFilterChanged(val) {
    ...
    $.ajax({
        ...
        contentType: "application/json; charset=utf-8",
        data: JSON.stringify({ ShowFilter: ShowFilter }),
        dataType: "json",
        async: true
    });
}
...
Grids.OnPaste = function (G, Cols, PastedText) {
    for (i = 0; i < PastedText.length; i++) {
        var val = PastedText[i].split('\t');
        for (j = 0; j < Cols.length; j++) {
            if (Cols[j].indexOf('Group') >= 0) {
                val[j] = val[j] + ' ';
            }
        }
        PastedText[i] = val.join('\t');
    }
    return false;
}