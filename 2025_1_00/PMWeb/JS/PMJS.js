...
function FormatNumbers() {
    ...
    //}
    $("input.ZeroTrimInteger").numeric({ AllowNegative: false }).blur(function () { if ($(this).val() == "0") $(this).val('') }).on("focus", function () { if ($(this).val() == "0") $(this).val('') });
    ...
    $("input.SpecDouble").numeric({ AllowNegative: true, DecimalCharacter: Global_DecimalCharacter, DecimalPrecision: Global_DecimalPrecision, DecimalSeparator: Global_DecimalSeparator });
    ...
    $("input.Days").numeric({ AllowNegative: true, DecimalCharacter: Global_DecimalCharacter, DecimalPrecision: Global_DecimalPrecision, DecimalSeparator: Global_DecimalSeparator }).blur(function (e) {
        if ($(this).val() % 1 == 0) {
            $(this).val(parseInt($(this).val()))
        }
    });
...
function ComboCheckAll(chk, comboId, hdnSelectedValuesId) {
    ...
    if (String(chk.id).includes("ProjectExplorer")) {
        SelectedItems = document.querySelectorAll(".SelectedItemClassNoImage");
        if (chk.checked) {
            combo.set_text(combo.get_items().get_count() - 1 + ' items selected');
            combo.set_value(values);
            $("#" + comboId)[0].classList.add("ItemsSelected");
            document.querySelector(".Clearddlbutton").style.display = "unset"
            if (hdnSelectedValues) hdnSelectedValues.value = values;
           
            SelectedItems.forEach(SelectedItem => {
                SelectedItem.classList.remove("SelectedItemClassNoImage");
            });
            chk.parentElement.parentElement.parentElement.classList.add("SelectedItemClassNoImage");    
        }
        else {
            combo.set_text("");
            combo.set_value("");
            $("#" + comboId)[0].classList.remove("ItemsSelected");
            document.querySelector(".Clearddlbutton").style.display = "none"
            if (hdnSelectedValues) hdnSelectedValues.value = "";
            SelectedItems.forEach(SelectedItem => {
                SelectedItem.classList.remove("SelectedItemClassNoImage");
            });
            chk.parentElement.parentElement.parentElement.classList.remove("SelectedItemClassNoImage");

        }
        /*   ProjectExplorerSelectedItems()*/
    }
    else {
        //        if (text.length > 0) {
        if (chk.checked) {
...
function ComboCheckParent(chk, comboId, hdnSelectedValuesId) {
    ...
    if (String(chk.id).includes("ProjectExplorer")) {
        ...
        text = NumberOfSelectedItems + ' items selected';
        chk.parentElement.parentElement.parentElement.classList.remove("SelectedItemClassNoImage")
        if (chk.checked) {
            chk.parentElement.parentElement.classList.add("SelectedItemClassNoImage");
        }
        else {
            chk.parentElement.parentElement.classList.remove("SelectedItemClassNoImage");
        }
...
function PMFormSubmit(PMForm, e) {
    ...
    if (EventTarget.length == 1 && EventTarget.val().endsWith('$btnPortfolio')) return true;
...
function RequestStart(sender, args) {
        ...
        //if (args.EventTarget.indexOf("ctl00$CPH1$DocumentAttachments$btnUpload") >= 0) {
        //    args.EnableAjax = false;
        //    return;
        //    }
         if (args.EventTarget.indexOf("$DocumentAttachments") >= 0 && args.EventTarget.indexOf("$btnUpload") >=0) {
             args.EnableAjax = false;
             return;
         }
        ...
        if (window.location.pathname.toLowerCase().indexOf("foldermanager.aspx") >= 0 && (args.EventTarget.indexOf("btnUpload") >= 0)) {
            args.EnableAjax = false;
            return;
        }
        ...
        if (args.EventTarget.indexOf("btnSearchDocumentExportExcel") >= 0) {
            args.EnableAjax = false;
            return;
        }
...
function BindPasteExcelHandlers(rdgCtrl) {
    ...
    var CanPaste = $("[id$=" + rdgCtrl + "]").find("a[id$=btnPasteClipBoard]").length > 0;
    if (clipboard && CanPaste) {
        ...
        $("[id$=" + rdgCtrl + "]").find(".rgCommandRow").focus(function () {CurrFocusedGrid = $(this).parents(".RadGrid")[0].id ; $('textarea[id$=txtClipboard]').focus(); });
        $("[id$=" + rdgCtrl + "]").find(".rgCommandRow").click(function () {
            CurrFocusedGrid = $(this).parents(".RadGrid")[0].id;
...
function UpdateClipboardData() {
    ...
    var btnPasteClipBoard;
    if (CurrFocusedGrid)
        btnPasteClipBoard = $("[id$=" + CurrFocusedGrid + "]").find("a[id$=btnPasteClipBoard]")[0];
    else
        btnPasteClipBoard = $("a[id$=btnPasteClipBoard]")[0]
...
function GetClipboardData(sender) {
    if (window.clipboardData) {
        ...
        alert(Msg_UseCtrl_V);
        if (sender && $(sender).parents('.RadGrid').length > 0)
            CurrFocusedGrid = $(sender).parents('.RadGrid')[0].id
        $('textarea[id$=txtClipboard]').focus();
        return false;
    }
}
...
function setCookie(c_name, value, expiredays) {
    ...
    ((expiredays == null) ? "" : ";expires=" + exdate.toGMTString()) + "; path=/; secure; samesite = lax;"
}
...
function setDirtyRadEditor(editor) {
    ...
    if ((editor.get_id() == 'ctl00_CPH1_edtMergeTemplate') ||
        ...
        (editor.get_id() == 'ctl00_CPH1_ActivityBoardsTemplate1_RadEditor1')
        ) {
        var objTree = $("[id$=tree]");
...
window.addEventListener('message', function (event) {
    if (event.data.event_id === 'toggleDirty') {
        toggleDirty(event.data.value);
    }

    if (event.data.event_id === 'toggleOverlay') {
        var backdrop = document.getElementsByClassName('ngOverlay')[0];
        var tblBreadCrumSegment = document.getElementsByClassName('tblBreadCrumSegment');
        var ExtensionToolbar = document.getElementsByClassName('ExtensionToolbar');
        var MasterToolbar = document.getElementsByClassName('MasterToolbar');
        if (event.data.value) {
            if (tblBreadCrumSegment.length > 0) {
                tblBreadCrumSegment[0].style.zIndex = 6000
                ExtensionToolbar[0].style.zIndex = 6000
                MasterToolbar[0].style.zIndex = 6000
            }
            if (backdrop.classList.contains('activated') == false) {
            ...
            if (tblBreadCrumSegment.length > 0) {
                tblBreadCrumSegment[0].style.zIndex = 7001
                ExtensionToolbar[0].style.zIndex = 7001
                MasterToolbar[0].style.zIndex = 7003
            }
        }
    }
});
...
function OpenUploadandViewAutodeskForgeFile(source, strguid, gridId) {
    ...
    var browserWidth = $telerik.$(window).width();
    var browserHeight = $telerik.$(window).height();
    var wnd = window.radopen('UploadAndViewAutodeskForgefilePopup.aspx?Source=' + source + '&FileGuid=' + strguid);
...
function Open3DViewer(ObjectType, ObjectId, FileGUID) {
    ...
    wnd.add_close(refreshAttachmentsPage);
    return false;
}
...
function OpenUpload3DViewer(ObjectType, ObjectId, FileGUID) {
    ...
    var wnd = window.parent.radopen('PMWeb3DViewer.aspx?ObjectType=' + ObjectType + '&ObjectId=' + ObjectId + '&FileGUID=' + FileGUID);
    var divWindow = wnd._popupElement;
    wnd.set_visibleTitlebar(false);
    wnd._topResizer.parentElement.className = "";
    divWindow.classList.add("rwFolderManager");
    wnd.setSize(1025, 800);
    wnd.Center();

    //To refresh the attachment grid when closing the popup
...
function refreshAttachmentsPage() {
    if (window.parent.document.querySelector(".Refresh")) {
        window.parent.document.querySelector(".Refresh").click();
    }
}
...
function OpenSpecNoteDetailPopup(txtNoteId) {
    var browserWidth = $telerik.$(window).width();
    var browserHeight = $telerik.$(window).height();
    var txt = document.getElementById(txtNoteId);
    var canEdit = true;
    if (txt)
        canEdit = !txt.disabled;
    var editMode = 1;
    if (!canEdit)
        editMode = 0;
    var wnd = window.radopen('NotesPopup.aspx?txtNotesId=' + txtNoteId + '&EditMode=' + editMode);
    if (isMobileScreen()) {
        wnd.setSize(browserWidth - 10, browserHeight - 10);
        wnd.moveTo(0, 0);
    }
...
function OpenviewAttachmentPopup(DocumentType, LineId, DocumentId, EntityTypeId, EntityId, IsLastRevision) {
    ...
    OpenPOPUpToSelfRedirect('ViewAttachments.aspx?DocumentType=' + DocumentType + '&LineId=' + LineId + '&DocumentId=' +
     DocumentId + '&EntityTypeId=' + EntityTypeId + '&EntityId=' + EntityId +
      '&IsLastRevision=' + IsLastRevision, 920, 500);
    return false;
}
function OpenSelectInspectionAssetPopupFromPWA(ProjectId, GridKeyName, InspectionId) {

    OpenPopupFromPWA('SelectAsset.aspx?Id=6&ProjectId=' + ProjectId + '&InspectionId=' + InspectionId + '&GridKeyName=' + GridKeyName, 1035, 710, GridKeyName);
    return false;
}
function OpenPopupFromPWA(URL, Width, Height, gridKeyName) {
    var browserWidth = $telerik.$(window).width();
    var browserHeight = $telerik.$(window).height();
    ...
    wnd.add_close(function (oWnd, args) {
        closePopupPWA(oWnd, args, gridKeyName);
    });
    return false;
}
function closePopupPWA(oWnd, args, gridKeyName) {
    const targetWindow = document.getElementById('ngFrame');
    if (targetWindow) {
        const messageData = {
            event_id: 'RebindGrid',
...
function OpenviewAttachmentPopupFromPWA(DocumentType, LineId, DocumentId, EntityTypeId, EntityId, IsLastRevision,gridKeyName,CanEdit) {

    OpenLineAttachmentFromPWA('ViewAttachments.aspx?DocumentType=' + DocumentType + '&LineId=' + LineId + '&DocumentId=' +
        DocumentId + '&EntityTypeId=' + EntityTypeId + '&EntityId=' + EntityId +
        '&IsLastRevision=' + IsLastRevision + '&GridKeyName=' + gridKeyName+'&CanEdit='+CanEdit, 920, 500, gridKeyName);
    return false;
}

function OpenCostWorksheetPopupFromPWA(ProjectId, CostCodeId,GridKeyName) {
    var browserWidth = $telerik.$(window).width();
    var browserHeight = $telerik.$(window).height();
    var wnd = window.radopen('CostWorksheetPopup.aspx?ProjectId=' + ProjectId + '&CostCodeId=' + CostCodeId +'&GridKeyName='+GridKeyName);
    if (isMobileScreen()) {
        wnd.setSize(browserWidth - 10, browserHeight - 10);
    }
    else {
...
function OpenLineAttachmentFromPWA(URL, Width, Height, gridKeyName) {
    var browserWidth = $telerik.$(window).width();
    var browserHeight = $telerik.$(window).height();
    var wnd = window.radopen(URL);
    if (isMobileScreen()) {
    ...
    wnd.add_close(function (oWnd, args) {
        closeAndRefreshPWAGrid(oWnd, args, gridKeyName);
    });
    return false;
}
function closeAndRefreshPWAGrid(oWnd, args, gridKeyName) {
    const targetWindow = document.getElementById('ngFrame');
    if (targetWindow) {
        const messageData = {
            event_id: 'RebindAttachmentLine',
            gridKeyName: gridKeyName
        };
        targetWindow.contentWindow.postMessage(messageData, '*');
    }
...
function sendMessageToAngular(eventId, data = {}) {
    const targetWindow = document.getElementById('ngFrame');
    if (targetWindow) {
        const messageData = {
            event_id: eventId,
...
function OpenNotificationPopupForPWA(RecordType, RecordId, RecordDescription, EntityId, EntityTypeId, CustomFormTypeId,AttachmentId, gridKeyName) {
    return OpenPopupFromPWA('Notification.aspx?ObjectType=' + RecordType + '&Id=' +
        RecordId + '&Description='
        + RecordDescription
        + '&RecordDescription=' + RecordDescription
        + '&EntityId=' + EntityId + '&EntityType=' + EntityTypeId
        + '&CustomFormTypeId=' + CustomFormTypeId
        + '&AttachmentId=' + AttachmentId, 500, 700, gridKeyName)
}

function OpenNotificationLogPopupForPWA(LogId, recordDescription, type, Id, CustomTypeId, canedit, EntityId, EntityType, gridKeyName) {
    recordDescription = recordDescription.replace("#", "").replace("'", "");
    return OpenPopupFromPWA('NotificationLog.aspx?ObjectType=' + type + '&Id=' + Id + '&LogId=' + LogId + '&RecordDescription=' + recordDescription + '&CustomFormTypeId=' + CustomTypeId + '&CanEdit=' + canedit + '&EntityId=' + EntityId + '&EntityType=' + EntityType, 500, 700, gridKeyName);
}
function OpenPopupAttachToEmail(url, width = 900, height = 600) {
    ...
    wnd.add_resizeEnd(function () {
        AdjustGridHeight(wnd);
    })
...
function OpenReminderPropupFromDatePicker(dtp, event) {
    ...
    OpenPOPUp("DefineReminderPopup.aspx?ProjectId=" +
                      ...
                      + LocationId
                      + "&ProgramId=" + ProgramId
                      + "&ObjectTypeId=" + ObjectTypeId + "&FieldId=" + FieldId + "&RecordId=" + RecordId + "&DocumentId=" + DocumentId + "&IsDetail=" + IsDetail + "&SpecFieldId=" + SpecFieldId + "&CustomFormFieldId=" + CustomFormFieldId + "&CustomFormColumnDataId=" + CustomFormColumnDataId, 477, 690, false);
...
function OpenReminderPropupFromAiAssistant(reminderElement) {
    var ProjectId = 0;
    var LocationId = 0;
    var ProgramId = 0;
    var ObjectTypeId = 0;
    ...
    if ($(reminderElement).attr("projectid") != null)
        ProjectId = $(reminderElement).attr("projectid");
    if ($(reminderElement).attr("locationid") != null)
        LocationId = $(reminderElement).attr("locationid");
    if ($(reminderElement).attr("programid") != null)
        ProgramId = $(reminderElement).attr("programid");
    if ($(reminderElement).attr("objecttypeid") != null)
        ObjectTypeId = $(reminderElement).attr("objecttypeid");
    if ($(reminderElement).attr("fieldid") != null)
        FieldId = $(reminderElement).attr("fieldid");
    if ($(reminderElement).attr("recordid") != null)
        RecordId = $(reminderElement).attr("recordId");
    if ($(reminderElement).attr("isdetail") != null)
        IsDetail = $(reminderElement).attr("isdetail");
    if ($(reminderElement).attr("documentid") != null)
        DocumentId = $(reminderElement).attr("documentid");
    if ($(reminderElement).attr("specfieldid") != null)
        SpecFieldId = $(reminderElement).attr("specfieldid");
    if ($(reminderElement).attr("customformfieldid") != null)
        CustomFormFieldId = $(reminderElement).attr("customformfieldid");
    if ($(reminderElement).attr("customformcolumndataid") != null)
        CustomFormColumnDataId = $(reminderElement).attr("customformcolumndataid");
    window.parent.OpenPOPUp("DefineReminderPopup.aspx?ProjectId=" +
        ProjectId + "&LocationId="
        + LocationId
        + "&ProgramId=" + ProgramId
        + "&ObjectTypeId=" + ObjectTypeId + "&FieldId=" + FieldId + "&RecordId=" + RecordId + "&DocumentId=" + DocumentId + "&IsDetail=" + IsDetail + "&SpecFieldId=" + SpecFieldId + "&CustomFormFieldId=" + CustomFormFieldId + "&CustomFormColumnDataId=" + CustomFormColumnDataId, 477, 690, false);
...
function ResourcesAllowPayTyeComboCheckAll(chk, comboId) {
    ...
    if (chkAll.checked) {
        ...
        $("#" + comboId + "_DropDown").find("input[id*='chkDefault']").each(function () {
            if ($("[id$=chkDefault]")[j].checked) {
                $("[id$=chkDefault]")[j].checked = false;
            }
            j++;
        });
        combo.set_text("");
...
function helpClick() {
    ...
    window.parent.open(window.parent.helpLink);
}
...
/************************************************/
    ...
    function PopupImageGallery(btnImg) {
        ...
        $('#RotatorContainer').find("input[type='image']").each(function () {
            if ($(btnImg).attr('ImageId') == $(this).attr('ImageId'))
                $('<img src="' + $(this).attr('src') + '" style="border-width:0px;max-height:320px;" >').prependTo("#PmMasterFader");
            else
                $('<img src="' + $(this).attr('src') + '" style="border-width:0px;max-height:320px;" >').appendTo("#PmMasterFader");
            count = count + 1;
    ...
    function JSEscape(strText){
        var strResult  = strText;
    ...
    function rdgAssignUserGroups_CheckboxCheckedChanged(sender) {
        var grid = $("[id$='rdgAssignUserGroups']");
        var row = $(sender).parents(".rgEditForm:first");
        if (!row || row.length == 0)
            row = $(sender).parents("tr:first");
        var chkDefault = row.find("input[id$='chkDefault']")[0];
        var chkShow = row.find("input[id$='chkShow']")[0];
        if (chkDefault.checked && sender == chkDefault) {
            chkShow.checked = true;
        }
        if (!chkShow.checked && sender == chkShow) {
...
function OpenRecentDocuments() {
    ...
    try {
        if (ObjectType) {
            OpenRecentDocumentsPopup(ObjectType);
        } else {
            OpenRecentDocumentsPopup('AllRecordTypes');
        }
    } catch (e) {
        OpenRecentDocumentsPopup('AllRecordTypes');
    }
...
function OpenRecentDocumentsPopup(objectType) {
    if (typeof HideFlyoutMenu == "function")
        HideFlyoutMenu();
    if (objectType != '' && wndRecentRecords == null) {
        var width = (screen.width >= 1400) ? 1350 : 1200;
        var left = (screen.width - 1200) / 2;
        var top = (screen.height - 700) / 2;
        window.open('RecentDocuments.aspx?objecttype=' + objectType, 'Recent', 'location=no,status=0,menubar=0,addressbar=0,resizable=1,scrollbars=1,height=700,top=' + top + ',left=' + left + ',width=' + width);
    }
}
