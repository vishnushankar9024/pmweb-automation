                                    ...
                                    PageMethods.SaveRecordMapSettings(objMapCenter.lat(), objMapCenter.lng(), objMap.getZoom(), $("input[id=txtlatitude]").val(), $("input[id=txtLongitude]").val(), $("input[id=txtElevation]").val(), $("input[id=txtGoogleAddress]").val(),
                                        function (response) {
                                            if (recordType == "FILEMANGER_LOOKUPFILES" || recordType == "ATTACHMENTS") {
                                                ...
                                                const targetWindow = window.parent.WebForm_GetElementById('ngFrame');
                                                if (targetWindow) {
                                                    var txtGeolocation = $("input[id=txtlatitude]").val() + ", " + $("input[id=txtLongitude]").val();
                                                    const messageData = {
                                                        event_id: 'UpdateGeolocation',
                                                        geolocation: txtGeolocation,
                                                        zoomlevel: objMap.getZoom(),
                                                        mapcenter: objMapCenter.lat() + ';' + objMapCenter.lng(),
                                                        elevation: $("input[id=txtElevation]").val(),
                                                    };
